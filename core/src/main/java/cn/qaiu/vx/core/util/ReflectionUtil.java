...
public final class ReflectionUtil {
        ...
        new ConfigurationBuilder()
            ...
            .setScanners(
                Scanners.SubTypes.filterResultsBy(
                    s -> true), // 允许getAllTypes获取所有Object的子类, 不设置为false则 getAllTypes
                // 会报错.默认为true.
                new MethodParameterNamesScanner(), // 设置方法参数名称 扫描器,否则调用getConstructorParamNames 会报错
                Scanners.MethodsAnnotated, // 设置方法注解 扫描器,
                // 否则getConstructorsAnnotatedWith,getMethodsAnnotatedWith 会报错
  ...
  public static boolean isBasicType(CtClass ctClass) {
    ...
    return ctClass
        .getName()
        .matches(
            "^java\\.lang\\.((Boolean)|(Character)|(Byte)|(Short)|(Integer)|(Long)|"
                + "(Float)|(Double)|(String))$");
  }
  ...
  public static boolean isBasicTypeArray(CtClass ctClass) {
    if (!ctClass.isArray()) {
      ...
      return (ctClass
          .getName()
          .matches("^(boolen|char|byte|short|int|long|float|double|String)\\[]$"));
  }
   ...
   */
  public static <T> T newWithNoParam(Class<T> handler)
   ...
   */
  public static Object invokeWithArguments(Method method, Object instance, Object... arguments)
      throws Throwable {
