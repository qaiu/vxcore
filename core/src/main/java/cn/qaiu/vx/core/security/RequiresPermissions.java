...
/**
 * 权限校验注解 支持权限字符串格式：resource:action，支持通配符 *
 *
 * <p>权限字符串格式：
 *
 * <ul>
 *   <li>{@code user:read} - 用户读取权限
 *   <li>{@code user:write} - 用户写入权限
 *   <li>{@code user:*} - 用户所有权限（通配符）
 *   <li>{@code *:read} - 所有资源的读取权限
 *   <li>{@code admin:*} - 管理员所有权限
 * </ul>
 *
 * <p>使用示例：
 *
