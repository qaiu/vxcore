...
/**
 * 自定义Service代理生成器
 * 基于@CustomProxyGen注解，生成适合当前项目的Service代理类
 * 支持接口继承树分析，生成所有父接口方法的代理
 * 
...
public class ServiceProxyGenerator extends AbstractProcessor {
            ...
            if (element.getKind() == ElementKind.INTERFACE) {
                try {
                    ...
                    messager.printMessage(Diagnostic.Kind.ERROR, 
                        "Failed to generate service proxy for " + element.getSimpleName() + ": " + e.getMessage());
                }
            }
        }
        return true;
    ...
    private void generateServiceProxy(TypeElement serviceInterface) throws IOException {
        ...
        messager.printMessage(Diagnostic.Kind.NOTE, 
            "Generated service proxy suite: " + packageName + "." + proxyClassName + " and " + handlerClassName);
    }

    /**
     * 生成代理类代码 - 完全兼容Vert.x ProxyGen格式
     */
    private void generateProxyClass(Writer writer, TypeElement serviceInterface, 
                                  String packageName, String className, String proxyClassName) throws IOException {
     ...
     * 递归收集接口方法
     */
    private void collectMethodsRecursively(TypeElement interfaceElement, 
    ...
    private void collectTypesFromType(TypeMirror type, Set<String> imports) {
        ...
        if (typeString.startsWith("io.vertx.core.Future<")) {
            String innerType = typeString.substring("io.vertx.core.Future<".length(), typeString.length() - 1);
            String concreteInnerType = replaceGenericTypes(innerType);
            if (!isPrimitiveOrBasicType(concreteInnerType) && !isGenericTypeParameter(concreteInnerType)) {
        ...
        // 处理LambdaPageResult<T>类型
            ...
            String innerType = typeString.substring("cn.qaiu.db.dsl.lambda.LambdaPageResult<".length(), typeString.length() - 1);
            String concreteInnerType = replaceGenericTypes(innerType);
            if (!isPrimitiveOrBasicType(concreteInnerType) && !isGenericTypeParameter(concreteInnerType)) {
                imports.add(concreteInnerType);
            }
     ...
     * 从父接口中提取具体类型
     */
    private String extractConcreteTypeFromSuperInterface(TypeElement serviceInterface, int typeParamIndex) {
        // 查找SimpleJService接口
    ...
    private void generateConstructors(Writer writer, String className) throws IOException {
        ...
        // 第二个构造函数
        writer.write("  public " + className + "VertxEBProxy(Vertx vertx, String address, DeliveryOptions options) {\n");
        writer.write("    this._vertx = vertx;\n");
        writer.write("    this._address = address;\n");
        writer.write("    this._options = options;\n");
        writer.write("    try {\n");
        writer.write("      this._vertx.eventBus().registerDefaultCodec(ServiceException.class, new ServiceExceptionMessageCodec());\n");
    ...
    private void generateMethod(Writer writer, ExecutableElement method) throws IOException {
        ...
        // 方法体 - 完全匹配Vert.x格式
        writer.write("    if (closed) return io.vertx.core.Future.failedFuture(\"Proxy is closed\");\n");
        writer.write("    JsonObject _json = new JsonObject();\n");
        
        // 添加参数到JSON
        for (VariableElement param : parameters) {
            writer.write("    _json.put(\"" + param.getSimpleName() + "\", " + param.getSimpleName() + ");\n");
        }
        writer.write("\n");
        
        // 设置DeliveryOptions
        writer.write("    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();\n");
        ...
        if (returnType.startsWith("io.vertx.core.Future<")) {
            String innerType = returnType.substring("io.vertx.core.Future<".length(), returnType.length() - 1);
            
            if (innerType.equals("java.util.List")) {
                writer.write("    return _vertx.eventBus().<JsonArray>request(_address, _json, _deliveryOptions).map(msg -> {\n");
                writer.write("      return ProxyUtils.convertList(msg.body().getList());\n");
            } else if (innerType.equals("io.vertx.core.json.JsonObject")) {
                writer.write("    return _vertx.eventBus().<JsonObject>request(_address, _json, _deliveryOptions).map(msg -> {\n");
                ...
                writer.write("    return _vertx.eventBus().<" + innerType + ">request(_address, _json, _deliveryOptions).map(msg -> {\n");
                writer.write("      return msg.body();\n");
            } else {
                // 实体类型
                writer.write("    return _vertx.eventBus().<" + innerType + ">request(_address, _json, _deliveryOptions).map(msg -> {\n");
            ...
            writer.write("    return io.vertx.core.Future.failedFuture(\"Unsupported return type: " + returnType + "\");\n");
        }
     ...
     */
    private void generateProxyHandlerClass(Writer writer, TypeElement serviceInterface, 
                                         String packageName, String className, String handlerClassName) throws IOException {
     ...
     * 生成Handler导入语句
     */
    private void generateHandlerImports(Writer writer, TypeElement serviceInterface) throws IOException {
        Set<String> imports = new LinkedHashSet<>();
     ...
     * 生成Handler构造函数
     */
    private void generateHandlerConstructors(Writer writer, String className, String handlerClassName) throws IOException {
        // 第一个构造函数
        ...
        // 第二个构造函数
        writer.write("  public " + handlerClassName + "(Vertx vertx, " + className + " service, long timeoutInSecond){\n");
        writer.write("    this(vertx, service, true, timeoutInSecond);\n");
        writer.write("  }\n\n");
        
        // 第三个构造函数
        writer.write("  public " + handlerClassName + "(Vertx vertx, " + className + " service, boolean topLevel, long timeoutInSecond){\n");
        writer.write("    this(vertx, service, true, timeoutInSecond, false);\n");
        writer.write("  }\n\n");
        
        // 第四个构造函数
        writer.write("  public " + handlerClassName + "(Vertx vertx, " + className + " service, boolean topLevel, long timeoutSeconds, boolean includeDebugInfo) {\n");
     ...
     * 生成Handler方法
     */
    private void generateHandlerMethods(Writer writer, TypeElement serviceInterface) throws IOException {
        ...
        writer.write("      if (action == null) throw new IllegalStateException(\"action not specified\");\n");
        writer.write("      accessed();\n");
        ...
        writer.write("        default: throw new IllegalStateException(\"Invalid action: \" + action);\n");
        writer.write("      }\n");
        writer.write("    } catch (Throwable t) {\n");
        writer.write("      if (includeDebugInfo) msg.reply(new ServiceException(500, t.getMessage(), HelperUtils.generateDebugInfo(t)));\n");
     ...
     * 生成Handler方法case
     */
    private void generateHandlerMethodCase(Writer writer, ExecutableElement method) throws IOException {
        String methodName = method.getSimpleName().toString();
        String returnType = replaceGenericTypes(method.getReturnType().toString());
        ...
        for (int i = 0; i < parameters.size(); i++) {
            ...
            if (paramType.equals("java.lang.String")) {
                ...
                writer.write("json.getValue(\"" + paramName + "\") == null ? null : (json.getLong(\"" + paramName + "\").longValue())");
            } else if (paramType.equals("java.lang.Integer") || paramType.equals("int")) {
                writer.write("json.getValue(\"" + paramName + "\") == null ? null : (json.getLong(\"" + paramName + "\").intValue())");
            } else if (paramType.equals("java.lang.Boolean") || paramType.equals("boolean")) {
                writer.write("json.getValue(\"" + paramName + "\") == null ? null : (json.getBoolean(\"" + paramName + "\"))");
            } else {
