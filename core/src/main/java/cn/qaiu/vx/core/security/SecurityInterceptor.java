...
public class SecurityInterceptor implements Handler<RoutingContext> {
    ...
    private SecurityRequirement getSecurityRequirement(RoutingContext ctx) {
        ...
        if (handlerClass instanceof Class<?>) {
            ...
            // 类级别权限（如果方法级别没有设置）
            if (requirement.getRequiredPermissions() == null || requirement.getRequiredPermissions().length == 0) {
    ...
    private static class SecurityRequirement {
        ...
        public RequiresPermissions.Logical getPermissionLogical() { return permissionLogical; }
        public void setPermissionLogical(RequiresPermissions.Logical permissionLogical) { this.permissionLogical = permissionLogical; }

        public RequiresPermissions.Logical getRoleLogical() { return roleLogical; }
        public void setRoleLogical(RequiresPermissions.Logical roleLogical) { this.roleLogical = roleLogical; }
    }
}
