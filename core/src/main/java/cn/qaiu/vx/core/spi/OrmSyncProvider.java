...
/**
 * ORM同步提供者SPI接口 由core-database模块实现，用于在框架启动时执行DDL同步
 *
...
public interface OrmSyncProvider {
  ...
  class OrmSyncResult {
    ...
    public OrmSyncResult(
        boolean success, int tablesCreated, int tablesUpdated, int tablesFailed, String message) {
    ...
    public static OrmSyncResult success(int created, int updated) {
      return new OrmSyncResult(
          true,
          created,
          updated,
          0,
          String.format(
              "DDL sync completed: %d tables created, %d tables updated", created, updated));
    }
