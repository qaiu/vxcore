...
public class CodeGenCli {
   ...
   * @return 生成结果
   */
  public CodeGenResult generateEntity(
      String tableName, String className, String packageName, String outputPath) {
    try {
      // 参数验证
   ...
   * @return 生成结果
   */
  public CodeGenResult generateDao(
      String tableName, String className, String packageName, String outputPath) {
    try {
      // 参数验证
      if (tableName == null || tableName.trim().isEmpty()) {
   ...
   * @return 生成结果
   */
  public CodeGenResult generateController(
      String tableName, String className, String packageName, String outputPath) {
    try {
      // 参数验证
      if (tableName == null || tableName.trim().isEmpty()) {
      ...
      String fullPath =
          outputPath
              + "/"
              + packageInfo.getControllerPackagePath()
              + "/"
              + className
              + "Controller.java";
      java.nio.file.Path filePath = java.nio.file.Paths.get(fullPath);
   ...
   * @return 生成结果
   */
  public CodeGenResult generateAll(
      String tableName, String className, String packageName, String outputPath) {
    try {
      // 参数验证
      if (tableName == null || tableName.trim().isEmpty()) {
      ...
      CodeGenResult controllerResult =
          generateController(tableName, className, packageName, outputPath);
  ...
  public static class CodeGenResult {
    ...
    public String toString() {
      return "CodeGenResult{" + "success=" + success + ", message='" + message + '\'' + '}';
    }
  }
}
