...
/**
 * Token黑名单管理器 用于管理已失效/登出的Token，防止Token在过期前被继续使用
 *
 * <p>特性：
 *
 * <ul>
...
public class TokenBlacklist {
  ...
  public void cleanup() {
    ...
    if (removed > 0) {
      LOGGER.debug(
          "TokenBlacklist cleanup completed, removed {} expired entries, remaining: {}",
          removed,
          blacklist.size());
    }
  }
  ...
  /**
   * 生成Token标识（用于没有JTI的情况） 使用Token内容的hash作为标识
   *
