...
public class ExceptionManager {
  ...
  private static final Map<Class<?>, List<ExceptionProcessor>> controllerProcessors =
      new HashMap<>();
  ...
  private static void initializeExceptionHandlers() {
    ...
    // 注册全局异常处理器
    Set<Class<?>> globalHandlerClasses =
        reflections.getTypesAnnotatedWith(GlobalExceptionHandler.class);
    for (Class<?> handlerClass : globalHandlerClasses) {
      try {
        ...
        LOGGER.error(
            "Failed to register global exception handler: {}", handlerClass.getSimpleName(), e);
      }
    }

    // 注册控制器异常处理器
    Set<Class<?>> controllerAdviceClasses =
        reflections.getTypesAnnotatedWith(ControllerAdvice.class);
    for (Class<?> adviceClass : controllerAdviceClasses) {
      try {
        ...
        LOGGER.error(
            "Failed to register controller exception handler: {}", adviceClass.getSimpleName(), e);
      }
    }
    ...
    controllerProcessors
        .values()
        .forEach(
            processors -> processors.sort(Comparator.comparingInt(ExceptionProcessor::getOrder)));

    LOGGER.info(
        "Exception handling initialized with {} global processors and {} controller processors",
        globalProcessors.size(),
        controllerProcessors.size());
  }
  ...
  private static void registerGlobalExceptionHandler(Object handlerInstance) {
    ...
    GlobalExceptionHandler classAnnotation =
        handlerInstance.getClass().getAnnotation(GlobalExceptionHandler.class);
      ...
      if (methodAnnotation != null) {
        ...
        for (Class<? extends Throwable> exceptionType : exceptionTypes) {
          ExceptionProcessor processor =
              new MethodBasedExceptionProcessor(handlerInstance, method, exceptionType, finalOrder);
  ...
  /** 注册控制器异常处理器 */
  private static void registerControllerExceptionHandler(
      Class<?> controllerClass, Object handlerInstance) {
    Method[] methods = handlerInstance.getClass().getMethods();
  ...
  public static boolean handleException(RoutingContext ctx, Throwable throwable) {
      ...
      if (processors != null) {
        for (ExceptionProcessor processor : processors) {
          if (processor.supports(throwable) && processor.handleException(ctx, throwable)) {
            LOGGER.debug(
                "Exception handled by controller processor: {}",
    ...
    for (ExceptionProcessor processor : globalProcessors) {
      if (processor.supports(throwable) && processor.handleException(ctx, throwable)) {
        LOGGER.debug(
            "Exception handled by global processor: {}", processor.getClass().getSimpleName());
        return true;
      }
    }
  ...
  private static class MethodBasedExceptionProcessor implements ExceptionProcessor {
    ...
    public MethodBasedExceptionProcessor(
    ...
    public boolean handleException(RoutingContext ctx, Throwable throwable) {
      try {
        Object result =
            ReflectionUtil.invokeWithArguments(handlerMethod, handlerInstance, ctx, throwable);
