...
public class CommonUtil {
  ...
  public static void initConfig(String configName, Class<?> tClass) {
    ...
    Buffer buffer =
        VertxHolder.getVertxInstance().fileSystem().readFileBlocking(resource.getPath());
  ...
  /**
   * 对类集合进行排序 根据 @HandleSortFilter 注解的 value 值进行排序
   *
   * @param set 要排序的类集合
   * @param <T> 类型参数
   * @return 排序后的实例集合
  ...
  /**
   * 获取应用版本号 从 app.properties 文件中读取版本信息
   *
   * @return 应用版本号字符串
   */
