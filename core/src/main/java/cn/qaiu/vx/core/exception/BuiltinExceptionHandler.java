...
public class BuiltinExceptionHandler implements ExceptionHandlerInterface {
  ...
  /** 处理验证异常 */
  private JsonResult<?> handleValidationException(
      ValidationException exception, RoutingContext ctx) {
  ...
  /** 处理非法参数异常 */
  private JsonResult<?> handleIllegalArgumentException(
      IllegalArgumentException exception, RoutingContext ctx) {
    return JsonResult.error("参数错误: " + exception.getMessage(), 400);
  }

  /** 处理空指针异常 */
  private JsonResult<?> handleNullPointerException(
      NullPointerException exception, RoutingContext ctx) {
