...
public class TableInfo {
  ...
  public List<ColumnInfo> getPrimaryKeyColumns() {
    return columns.stream().filter(ColumnInfo::isPrimaryKey).toList();
  }
  ...
  public List<ColumnInfo> getNonPrimaryKeyColumns() {
    return columns.stream().filter(column -> !column.isPrimaryKey()).toList();
  }
  ...
  public List<ColumnInfo> getColumnsByType(String columnType) {
    return columns.stream().filter(column -> columnType.equals(column.getColumnType())).toList();
  }
  ...
  public List<IndexInfo> getUniqueIndexes() {
    return indexes.stream().filter(IndexInfo::isUnique).toList();
  }
  ...
  public List<IndexInfo> getNormalIndexes() {
    return indexes.stream().filter(index -> !index.isUnique()).toList();
  }
