...
public class SecurityContext {
  ...
  /**
   * 检查是否有指定权限 支持通配符匹配：user:* 匹配 user:read, user:write 等
   *
  ...
  private boolean checkWildcardPermission(String requiredPermission) {
    ...
    for (String userPermission : permissions) {
      ...
      // 检查资源匹配
      boolean resourceMatch = "*".equals(userResource) || userResource.equals(requiredResource);

      // 检查操作匹配
      boolean actionMatch = "*".equals(userAction) || userAction.equals(requiredAction);
