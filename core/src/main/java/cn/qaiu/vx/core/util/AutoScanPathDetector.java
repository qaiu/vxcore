...
public class AutoScanPathDetector {
  ...
  public static Set<String> detectScanPaths(Class<?> mainClass) {
    ...
    if (mainClass.isAnnotationPresent(App.class)) {
      ...
      if (baseScanPackage != null && !baseScanPackage.trim().isEmpty()) {
        ...
        LOGGER.info(
            "@App annotation found but baseScanPackage is empty, falling back to auto-detection");
      }
    }
    ...
    if (packageName == null || packageName.isEmpty()) {
      LOGGER.warn(
          "Cannot determine package name from main class: {}, using default", mainClassName);
      scanPaths.add("cn.qaiu");
      return scanPaths;
    }
  ...
  public static String getRecommendation(Class<?> mainClass) {
    ...
    if (mainClass.isAnnotationPresent(App.class)) {
      ...
      if (baseScanPackage != null && !baseScanPackage.trim().isEmpty()) {
        recommendation
            .append("App-annotated scan paths for ")
            .append(mainClass.getName())
            .append(":\n");
      } else {
        recommendation
            .append("Auto-detected scan paths for ")
            .append(mainClass.getName())
            .append(":\n");
      }
    } else {
