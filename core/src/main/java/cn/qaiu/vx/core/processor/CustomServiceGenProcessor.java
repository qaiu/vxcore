 ...
 * 自定义服务生成注解处理器
 *
 * <p>功能： 1. 动态读取父接口的所有方法和泛型参数 2. 根据方法和泛型参数生成具体的服务类 3. 支持通用的 ProxyGen 注解异步服务类生成 4. 生成的服务类支持 Vert.x
 * 的异步编程模型
 *
 * <p>使用方式： 在实体类上添加 @GenerateServiceGen 注解，处理器会自动生成对应的服务接口和实现类
 *
 * @author vxcore
 * @version 1.0
 */
...
public class CustomServiceGenProcessor extends AbstractProcessor {
    ...
    for (Element element : roundEnv.getElementsAnnotatedWith(GenerateServiceGen.class)) {
      ...
      try {
        ...
        messager.printMessage(
            ...
            "Failed to generate service classes for "
   ...
   */
  private void generateServiceClasses(TypeElement entityElement, GenerateServiceGen annotation)
      throws IOException {
    ...
    if (entityElement.getKind() == ElementKind.INTERFACE) {
      ...
      if (annotation.generateProxy()) {
        generateServiceInterfaceFromInterface(
            basePackage, serviceName, entityName, entityPackage, interfaceMethods);
      }
      ...
      if (referenceInterface != Void.class && referenceInterface != TestReferenceInterface.class) {
        // 使用参照接口生成方法
        List<MethodInfo> referenceMethods =
            analyzeReferenceInterfaceMethods(referenceInterface, entityElement);
        messager.printMessage(
            Diagnostic.Kind.NOTE,
        ...
        // 生成 ProxyGen 接口
        if (annotation.generateProxy()) {
          generateServiceInterfaceFromReferenceInterface(
              basePackage, serviceName, entityName, entityPackage, referenceMethods);
        }
        ...
        if (annotation.generateProxy()) {
          generateServiceInterfaceFromReferenceInterface(
              basePackage, serviceName, entityName, entityPackage, builtInMethods);
        }
        ...
        if (annotation.generateProxy()) {
          generateServiceInterface(
              basePackage, serviceName, entityName, entityPackage, genericTypes);
        }
  ...
  private Class<?> getReferenceInterface(GenerateServiceGen annotation) {
    try {
      ...
      String typeName = e.getTypeMirror().toString();
      messager.printMessage(Diagnostic.Kind.NOTE, "Reference interface type name: " + typeName);
   ...
   * @return 方法信息列表
   */
  private List<MethodInfo> analyzeReferenceInterfaceMethods(
      Class<?> referenceInterface, TypeElement entityElement) {
    List<MethodInfo> methods = new ArrayList<>();
    ...
    try {
      ...
      messager.printMessage(
          Diagnostic.Kind.WARNING,
          "Failed to analyze reference interface "
   ...
   * @return 替换后的方法列表
   */
  private List<MethodInfo> replaceGenericParametersInMethods(
      List<MethodInfo> methods, TypeElement entityElement) {
   ...
   */
  private void collectMethodsFromInterfaces(
      TypeElement interfaceElement, List<MethodInfo> methods, Set<String> methodSignatures) {
    // 收集当前接口的方法
  ...
  private MethodInfo createMethodInfo(
      ExecutableElement methodElement, TypeElement interfaceElement) {
   ...
   * @throws IOException 文件操作异常
   */
  private void generateServiceInterfaceFromReferenceInterface(
      String basePackage,
      String serviceName,
      String entityName,
   ...
   * @param referenceMethods 参照接口方法列表
   */
  private void generateReferenceInterfaceMethods(
      PrintWriter writer, List<MethodInfo> referenceMethods) {
   ...
   * @throws IOException 文件操作异常
   */
  private void generateServiceInterfaceFromInterface(
      String basePackage,
      String serviceName,
      String entityName,
      String entityPackage,
   ...
   * @param genericTypes 泛型类型列表
   */
  private void generateBasicCrudMethods(
      PrintWriter writer, String serviceName, String entityName, List<String> genericTypes) {
    // 创建方法 - 使用 Future 返回类型
    writer.println("    /**");
    writer.println("     * 创建实体");
   ...
   * @param genericTypes 泛型类型列表
   */
  private void generateCustomQueryMethods(
      PrintWriter writer, String serviceName, List<String> genericTypes) {
    writer.println("    /**");
    ...
    for (String genericType : genericTypes) {
      ...
      writer.println("     */");
      writer.println(
          "    Future<List<JsonObject>> findBy"
              + genericType
              + "(JsonObject "
              + genericType.toLowerCase()
              + ");");
      writer.println();
    }
  }
