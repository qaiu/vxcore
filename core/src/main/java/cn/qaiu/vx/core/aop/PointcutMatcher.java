 ...
 * 切点匹配器
 *
 * <p>解析切点表达式并判断方法是否匹配。 支持的表达式格式:
 *
 * <ul>
 *   <li>execution(modifiers? return-type declaring-type? method-name(params))
 *   <li>@annotation(fully.qualified.AnnotationName)
 *   <li>within(type-pattern)
 * </ul>
 *
 * @author qaiu
 * @since 1.0.0
...
public class PointcutMatcher {
  ...
  private static final Pattern ANNOTATION_PATTERN =
      Pattern.compile("@annotation\\s*\\(\\s*([\\w.]+)\\s*\\)");

  // within 表达式模式: within(cn.qaiu.*.service.*)
  private static final Pattern WITHIN_PATTERN =
      Pattern.compile("within\\s*\\(\\s*([\\w.*]+)\\s*\\)");
  ...
  /** 加载注解类，支持内部类 如果类名使用 . 分隔内部类名，会尝试将最后的 . 替换为 $ 来加载内部类 */
  @SuppressWarnings("unchecked")
