...
/**
 * 框架生命周期管理器 使用组合模式管理框架的启动、配置、服务注册等生命周期
 *
...
public class FrameworkLifecycleManager {
  ...
  private static final AtomicReference<FrameworkLifecycleManager> INSTANCE =
      new AtomicReference<>();
  ...
  public static FrameworkLifecycleManager getInstance() {
    return INSTANCE.updateAndGet(
        manager -> manager == null ? new FrameworkLifecycleManager() : manager);
  }
    ...
    return Future.future(
        promise -> {
          ...
          deploymentFutures.add(
              vertx.deployVerticle(RouterVerticle.class, getDeploymentOptions("Router")));
          deploymentFutures.add(
              vertx.deployVerticle(ServiceVerticle.class, getDeploymentOptions("Service")));
          ...
          if (proxyConfig != null && proxyConfig.getBoolean("enabled", false)) {
            deploymentFutures.add(
                vertx.deployVerticle(ReverseProxyVerticle.class, getDeploymentOptions("Proxy")));
          }

          // 部署后置执行Verticle
          deploymentFutures.add(
              vertx.deployVerticle(PostExecVerticle.class, getDeploymentOptions("PostExec", 2)));
  ...
  private Future<Void> executeUserCallback() {
    return Future.future(
        promise -> {
          if (userHandler != null) {
            vertx
                .createSharedWorkerExecutor("user-handler")
                .executeBlocking(
                    () -> {
