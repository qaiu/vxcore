...
/**
 * WebSocket处理器工厂 负责扫描和注册WebSocket处理器
 *
 * @author QAIU
 */
public class WebSocketHandlerFactory {
  ...
  /** 注册单个处理器到路由器 */
  private void registerHandlerToRouter(
      Router router, WebSocketHandlerInfo handlerInfo, String gatewayPrefix) {
  ...
  private void handleWebSocketConnection(
      io.vertx.ext.web.handler.sockjs.SockJSSocket socket, WebSocketHandlerInfo handlerInfo) {
    ...
    socket.closeHandler(
        v -> {
          if (handlerInfo.getOnCloseMethod() != null) {
            invokeEventMethod(
                handlerInfo.getOnCloseMethod(), handlerInfo.getInstance(), socket, null);
          }
        });
  ...
  private void handleMessage(
      io.vertx.ext.web.handler.sockjs.SockJSSocket socket,
      WebSocketHandlerInfo handlerInfo,
      String message,
  ...
  /** 调用事件方法 */
  private void invokeEventMethod(
      Method method,
      Object instance,
      io.vertx.ext.web.handler.sockjs.SockJSSocket socket,
      Object data) {
  ...
  public static class WebSocketHandlerInfo {
    ...
    public List<Method> getOnMessageMethods() {
      return onMessageMethods;
    }

    public void addOnMessageMethod(Method method) {
      this.onMessageMethods.add(method);
    }
    ...
    public List<Method> getOnErrorMethods() {
      return onErrorMethods;
    }

    public void addOnErrorMethod(Method method) {
      this.onErrorMethods.add(method);
    }
  }
}
