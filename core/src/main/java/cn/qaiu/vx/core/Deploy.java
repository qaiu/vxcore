...
public final class Deploy {
  ...
  /** 确定配置文件路径 支持新的 application.yml 格式和旧的 app-dev.yml 格式 */
  ...
  private void readConf(JsonObject conf) {
    ...
    if (conf.containsKey("server") && conf.containsKey("datasources")) {
      ...
      ConfigUtil.readYamlConfig(path + "-" + activeMode, tempVertx)
          .onSuccess(
              res -> {
  ...
  private void deployWithNewConfig() {
    ...
    var future0 =
        vertx
            .createSharedWorkerExecutor("other-handle")
            .executeBlocking(
                () -> {
    ...
    future0
        .onSuccess(
            res -> {
              ...
              // 部署基础服务
              var future1 =
                  vertx.deployVerticle(RouterVerticle.class, getWorkDeploymentOptions("Router"));
              var future2 =
              ...
              if (proxyConfig != null && proxyConfig.getBoolean("enabled", false)) {
                var future3 =
                    vertx.deployVerticle(
                        ReverseProxyVerticle.class, getWorkDeploymentOptions("proxy"));
  ...
  private void deployWithOldConfig() {
    ...
    var vertxOptions = vertxConfigELPS == 0 ? new VertxOptions() : new VertxOptions(vertxConfig);
    ...
    vertxOptions.setAddressResolverOptions(
        new AddressResolverOptions()
            ...
            .addServer("8.8.4.4"));
    LOGGER.info(
        "vertxConfigEventLoopPoolSize: {}, eventLoopPoolSize: {}, workerPoolSize: {}",
        vertxConfigELPS,
        ...
        .onSuccess(
            res -> {
              ...
              JsonObject jsonObject =
                  ((JsonObject) localMap.get(GLOBAL_CONFIG)).getJsonObject("proxy-server");
              if (jsonObject != null) {
                genPwd(jsonObject);
                var future4 =
                    vertx.deployVerticle(
                        HttpProxyVerticle.class, getWorkDeploymentOptions("proxy"));
  ...
  private void deployWorkVerticalSuccess(CompositeFuture compositeFuture) {
    ...
    var vertx = VertxHolder.getVertxInstance();
    var postExecFuture =
        vertx.deployVerticle(PostExecVerticle.class, getWorkDeploymentOptions("postExec", 2));
    ...
    var t2 =
        ((double) System.currentTimeMillis() - ManagementFactory.getRuntimeMXBean().getStartTime())
            / 1000;
