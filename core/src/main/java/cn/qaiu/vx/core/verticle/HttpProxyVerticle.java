...
public class HttpProxyVerticle extends AbstractVerticle {
        ...
        .onFailure(
            connectionAttempt -> {
              ...
              clientRequest
                  .response()
                  .setStatusCode(502)
                  .end("Bad Gateway: Unable to connect to target");
            });
  }
  ...
  private void handleClientRequest(HttpServerRequest clientRequest) {
    // 打印来源ip和访问目标URI
    LOGGER.debug(
        "source: {}, target: {}", clientRequest.remoteAddress().toString(), clientRequest.uri());
    if (proxyServerConf.containsKey("username")
        && StringUtils.isNotBlank(proxyServerConf.getString("username"))) {
  ...
  private void handleHttpRequest(HttpServerRequest clientRequest) {
    ...
    httpClient
        .request(clientRequest.method(), targetPort, targetHost, clientRequest.uri())
        .onSuccess(
            request -> {
              ...
              clientRequest
                  .headers()
                  .forEach(header -> request.putHeader(header.getKey(), header.getValue()));

              // 将客户端请求的 body 转发给目标服务器
              clientRequest.bodyHandler(
                  body ->
                      request
                          .send(body)
                          .onSuccess(
                                    ...
                                    .onFailure(
                                        err ->
                                            clientRequest
                                                .response()
                                                .setStatusCode(502)
                                                .end("Bad Gateway: Unable to reach target"));
                              })
                          .onFailure(
                              err ->
                                  clientRequest
