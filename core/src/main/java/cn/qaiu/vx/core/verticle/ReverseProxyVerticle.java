...
public class ReverseProxyVerticle extends AbstractVerticle {
  ...
  private static final String PATH_PROXY_CONFIG =
      SharedDataUtil.getJsonConfig(ConfigConstant.GLOBAL_CONFIG).getString("proxyConf");
  ...
  private void handleProxyConf(JsonObject proxyConf) {
    // page404 path
    if (proxyConf.containsKey("page404")) {
  ...
  private void handleWebSocketUpgrade(
      io.vertx.core.http.ServerWebSocket webSocket, JsonObject proxyConf) {
    String requestPath = webSocket.path();
    LOGGER.debug("WebSocket upgrade request: {}", requestPath);

    // 获取WebSocket代理配置
    @SuppressWarnings("unchecked")
    List<WebSocketProxyConfig> configs =
        (List<WebSocketProxyConfig>)
    ...
    proxyHandler.handleWebSocketProxy(
        webSocket, matchedConfig.getOrigin(), matchedConfig.getTargetPath());
  }
  ...
  private HttpServer getHttpsServer(JsonObject proxyConf) {
    ...
    if (proxyConf.containsKey("ssl")) {
      ...
      if (sslConfig.containsKey("enable") && sslConfig.getBoolean("enable")) {
        ...
        String sslCertificateKeyPath =
            sslUrl.getPath() + sslConfig.getString("ssl_certificate_key");
            ...
            ctx -> {
              if (staticConf.containsKey("add-headers")) {
                Map<String, String> headers =
                    CastUtil.cast(staticConf.getJsonObject("add-headers").getMap());
            ...
            location -> {
              ...
              try {
                ...
                if (StringUtils.isEmpty(originPath) || path.equals(originPath)) {
                  Route route =
                      path.startsWith("~")
                          ? proxyRouter.routeWithRegex(path.substring(1))
                  ...
                  //                    String uniqueSuffix =
                  // Integer.toHexString(uniqueKey.hashCode());
                  //
                  //// 规格化 originPath
                  ...
                  //
                  //// 唯一内部挂载前缀
                  //                    final String originMount = REROUTE_PATH_PREFIX +
                  // uniqueSuffix + originPath;
                  //
                  ...
                  //                        if (indexFile != null && (uri.equals(path0) ||
                  // uri.equals(path0.substring(0, path0.length()-1)))) {
                  //                            String rePath = originMount.endsWith("/") ?
                  // (originMount + indexFile.substring(1)) : (originMount + indexFile);
                  //                            ctx.reroute(rePath);
                  //                            return;
                  //                        }
                  //
                  //                        // 一般穿透：/n2/xxx -> originMount + xxx
                  //                        String rePath = uri.replaceFirst("^" + path0,
                  // originMount);
                  //                        ctx.reroute(rePath);
                  //                    });
                }
