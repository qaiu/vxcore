...
/**
 * 配置别名注册表 支持配置键的别名映射，如 jdbcUrl ↔ url，实现配置格式兼容
 *
...
public class ConfigAliasRegistry {
  ...
  /** 别名映射表：key -> 所有别名集合 例如: "jdbcUrl" -> ["jdbcUrl", "url", "jdbc-url"] */
  ...
  private void registerDefaultAliases() {
    ...
    // 驱动类别名
    registerAliasGroup(
        "driverClassName", "driverClassName", "driver", "driver-class-name", "driverClass");

    // 连接池大小别名
    registerAliasGroup(
        "maxPoolSize", "maxPoolSize", "max-pool-size", "max_pool_size", "maximumPoolSize");
    registerAliasGroup(
        "minPoolSize", "minPoolSize", "min-pool-size", "min_pool_size", "minimumPoolSize");
    ...
    // 超时配置别名
    registerAliasGroup(
        "timeout", "timeout", "timeOut", "time-out", "connectionTimeout", "connection-timeout");
  ...
  public Set<String> getAliases(String canonicalName) {
    ...
    return aliases != null
        ...
        : Collections.singleton(canonicalName);
  }
