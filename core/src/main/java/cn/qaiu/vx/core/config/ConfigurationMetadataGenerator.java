...
/**
 * 配置元数据生成器 扫描配置类并生成元数据信息，用于IDE自动提示
 *
 * @author QAIU
 */
public class ConfigurationMetadataGenerator {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(ConfigurationMetadataGenerator.class);
  ...
  public static class PropertyMetadata {
    ...
    public ConfigurationProperty.PropertyType getType() {
      return type;
    }

    public void setType(ConfigurationProperty.PropertyType type) {
      this.type = type;
    }
  ...
  public static List<ConfigurationClassMetadata> generateAllMetadata() {
    ...
    for (Class<?> configClass : configClasses) {
      try {
        ...
        LOGGER.error(
            "Failed to generate metadata for configuration class: {}",
  ...
  public static ConfigurationClassMetadata generateMetadata(Class<?> configClass) {
    ...
    ConfigurationProperties classAnnotation =
    ...
    for (ConfigurationClassMetadata classMetadata : metadataList) {
      ...
      for (PropertyMetadata property : classMetadata.getProperties()) {
        ...
        if (!property.getDefaultValue().isEmpty()) {
          propertySchema.put(
              "default", parseDefaultValue(property.getDefaultValue(), property.getType()));
        }
      ...
      if (!classMetadata.getPrefix().isEmpty()) {
        properties.put(
            classMetadata.getPrefix(),
            new JsonObject().put("$ref", "#/definitions/" + classMetadata.getClassName()));
      }
    }
  ...
  private static Object parseDefaultValue(
      String defaultValue, ConfigurationProperty.PropertyType type) {
