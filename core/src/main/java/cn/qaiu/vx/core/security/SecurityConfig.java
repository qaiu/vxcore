...
public class SecurityConfig {
    ...
    public SecurityConfig(JsonObject json) {
        if (json != null) {
            ...
            this.jwtPrivateKeyPath = json.getString("jwt-private-key", json.getString("jwtPrivateKeyPath"));
            this.jwtAlgorithm = json.getString("jwt-algorithm", json.getString("jwtAlgorithm", "RS256"));
            this.jwtExpireSeconds = json.getInteger("jwt-expire-seconds", json.getInteger("jwtExpireSeconds", 3600));
            this.refreshTokenExpireSeconds = json.getInteger("refresh-token-expire-seconds", 
            ...
            if (json.containsKey("jwt-auth-reg")) {
                this.authPaths = json.getJsonArray("jwt-auth-reg").stream()
                        .map(Object::toString)
                        .toArray(String[]::new);
            } else if (json.containsKey("authPaths")) {
                this.authPaths = json.getJsonArray("authPaths").stream()
                        .map(Object::toString)
                        .toArray(String[]::new);
            }
            if (json.containsKey("jwt-ignores-reg")) {
                this.ignorePaths = json.getJsonArray("jwt-ignores-reg").stream()
                        .map(Object::toString)
                        .toArray(String[]::new);
            } else if (json.containsKey("ignorePaths")) {
                this.ignorePaths = json.getJsonArray("ignorePaths").stream()
                        .map(Object::toString)
                        .toArray(String[]::new);
            }

            // Token配置
            this.tokenHeader = json.getString("token-header", json.getString("tokenHeader", "Authorization"));
    ...
    public static SecurityConfig fromJson(JsonObject json) {
        ...
        config.setJwtPublicKeyPath(json.getString("jwt-public-key", json.getString("jwtPublicKeyPath")));
        config.setJwtPrivateKeyPath(json.getString("jwt-private-key", json.getString("jwtPrivateKeyPath")));
        config.setJwtAlgorithm(json.getString("jwt-algorithm", json.getString("jwtAlgorithm", "RS256")));
        config.setJwtExpireSeconds(json.getInteger("jwt-expire-seconds", json.getInteger("jwtExpireSeconds", 3600)));
        config.setRefreshTokenExpireSeconds(json.getInteger("refresh-token-expire-seconds", 
                json.getInteger("refreshTokenExpireSeconds", 86400 * 7)));
        config.setJwtIssuer(json.getString("jwt-issuer", json.getString("jwtIssuer", "vxcore")));

        // 路径配置
        if (json.containsKey("jwt-auth-reg")) {
            config.setAuthPaths(json.getJsonArray("jwt-auth-reg").stream()
                    .map(Object::toString)
                    .toArray(String[]::new));
        }
        if (json.containsKey("jwt-ignores-reg")) {
            config.setIgnorePaths(json.getJsonArray("jwt-ignores-reg").stream()
                    .map(Object::toString)
                    .toArray(String[]::new));
        }

        // Token配置
        config.setTokenHeader(json.getString("token-header", json.getString("tokenHeader", "Authorization")));
        config.setTokenPrefix(json.getString("token-prefix", json.getString("tokenPrefix", "Bearer ")));

        // 防盗链配置
        config.setRefererEnabled(json.getBoolean("ref-enable", json.getBoolean("refererEnabled", false)));
