 ...
 * 切面处理器
 *
 * <p>使用 Byte Buddy 实现动态代理，将切面逻辑织入目标类。 支持两种模式：
 *
 * <ul>
 *   <li>代理模式：创建目标类的子类代理
 *   <li>重定义模式：使用 Java Agent 重新定义已加载的类（需要 byte-buddy-agent）
 * </ul>
 *
 * @author qaiu
 * @since 1.0.0
...
public class AspectProcessor {
   ...
   * 安装 Byte Buddy Agent
   *
   * <p>必须在任何类重定义操作之前调用。 可以在应用启动时调用，或通过 -javaagent 参数启动。
   */
  ...
  public <T> T createProxy(T target) {
    ...
    try {
      ...
      Class<?> proxyClass = unloaded.load(targetClass.getClassLoader()).getLoaded();
   ...
   * 使用 Agent 重定义类（运行时织入）
   *
   * <p>需要先调用 {@link #installAgent()} 安装 Agent。
   *
   * @param targetClass 目标类
   */
  ...
  /** 调用环绕通知 */
  private Object invokeAroundAdvice(
      Object aspectInstance, Method adviceMethod, ProceedingJoinPoint joinPoint) throws Throwable {
    try {
      Class<?>[] paramTypes = adviceMethod.getParameterTypes();
      Object[] adviceArgs = new Object[paramTypes.length];
