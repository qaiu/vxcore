...
public class ExceptionHandlerManager {
  ...
  // 全局异常处理器
  private final Map<Class<? extends Throwable>, ExceptionHandlerInterface> globalHandlers =
      new ConcurrentHashMap<>();

  // 局部异常处理器 (Controller -> ExceptionType -> Handler)
  private final Map<Class<?>, Map<Class<? extends Throwable>, LocalExceptionHandlerWrapper>>
      localHandlers = new ConcurrentHashMap<>();

  /** 注册全局异常处理器 */
  public void registerGlobalHandler(
      Class<? extends Throwable> exceptionType, ExceptionHandlerInterface handler) {
  ...
  public void registerLocalHandler(Class<?> controllerClass, Method handlerMethod) {
    ...
    Map<Class<? extends Throwable>, LocalExceptionHandlerWrapper> controllerHandlers =
        localHandlers.get(controllerClass);

    for (Class<? extends Throwable> exceptionType : exceptionTypes) {
      LocalExceptionHandlerWrapper wrapper =
          new LocalExceptionHandlerWrapper(controllerClass, handlerMethod, annotation.priority());
      controllerHandlers.put(exceptionType, wrapper);
      LOGGER.debug(
          "Registered local exception handler for {} in {}: {}",
  ...
  /** 使用局部异常处理器处理异常 */
  private JsonResult<?> handleWithLocalHandlers(
      Throwable exception, RoutingContext ctx, Object controllerInstance) {
  ...
  /** 查找最佳匹配的局部异常处理器 */
  private LocalExceptionHandlerWrapper findBestMatch(
      Throwable exception, Map<Class<? extends Throwable>, LocalExceptionHandlerWrapper> handlers) {
  ...
  private static class LocalExceptionHandlerWrapper {
    ...
    public LocalExceptionHandlerWrapper(
        Class<?> controllerClass, Method handlerMethod, int priority) {
      this.controllerClass = controllerClass;
      this.handlerMethod = handlerMethod;
      this.priority = priority;
    }

    public JsonResult<?> handle(Throwable exception, RoutingContext ctx, Object controllerInstance)
        throws Throwable {
      try {
        Object result =
            ReflectionUtil.invokeWithArguments(handlerMethod, controllerInstance, exception, ctx);
