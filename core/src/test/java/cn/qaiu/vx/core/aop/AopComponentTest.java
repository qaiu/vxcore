...
public class AopComponentTest {
  ...
  void testImplementsLifecycleComponent() {
    assertTrue(
        aopComponent instanceof LifecycleComponent, "AopComponent 应该实现 LifecycleComponent 接口");
  }
  ...
  void testInitializeWithAopEnabled(Vertx vertx, VertxTestContext testContext)
  ...
  void testInitializeWithAopDisabled(Vertx vertx, VertxTestContext testContext)
      throws InterruptedException {
    JsonObject config = new JsonObject().put("aop", new JsonObject().put("enabled", false));
  ...
  void testInitializeWithDefaultConfig(Vertx vertx, VertxTestContext testContext)
      throws InterruptedException {
  ...
  void testStart(Vertx vertx, VertxTestContext testContext) throws InterruptedException {
    JsonObject config = new JsonObject().put("aop", new JsonObject().put("enabled", true));
  ...
  void testGetAopConfig(Vertx vertx, VertxTestContext testContext) throws InterruptedException {
    JsonObject aopConfig = new JsonObject().put("enabled", true).put("custom-key", "custom-value");
  ...
  void testStartWhenDisabled(Vertx vertx, VertxTestContext testContext)
      throws InterruptedException {
    JsonObject config = new JsonObject().put("aop", new JsonObject().put("enabled", false));
