...
/**
 * HttpProxyVerticle测试类 测试HTTP代理服务器功能
 *
...
public class HttpProxyVerticleTest {
  ...
  void setUp(Vertx vertx, VertxTestContext testContext) {
    ...
    JsonObject config =
        new JsonObject()
            .put("proxy-server", new JsonObject().put("port", PROXY_PORT))
            .put("proxy-pre", new JsonObject());

    vertx.sharedData().getLocalMap(ConfigConstant.LOCAL).put(ConfigConstant.GLOBAL_CONFIG, config);
  ...
  void testDeploySuccess(Vertx vertx, VertxTestContext testContext) {
    vertx
        .deployVerticle(new HttpProxyVerticle())
        .onComplete(
                ...
                error -> {
                  testContext.verify(
                      () -> {
                        ...
                        assertTrue(
                            error.getMessage().contains("Global configuration"),
                ...
                error -> {
                  testContext.verify(
                      () -> {
                        assertNotNull(error, "应该有错误信息");
                        assertTrue(
                            error.getMessage().contains("proxy-server"), "错误信息应包含'proxy-server'");
                      });
                  testContext.completeNow();
                }));
  }
  ...
  void testExtractPortFromHttpUrl() {
    assertEquals(
        8080, HttpProxyVerticle.extractPortFromUrl("http://example.com:8080/path"), "应该提取到端口8080");
    assertEquals(
        80, HttpProxyVerticle.extractPortFromUrl("http://example.com/path"), "HTTP默认端口应为80");
  }
  ...
  void testExtractPortFromHttpsUrl() {
    assertEquals(
        8443, HttpProxyVerticle.extractPortFromUrl("https://example.com:8443/path"), "应该提取到端口8443");
    assertEquals(
        443, HttpProxyVerticle.extractPortFromUrl("https://example.com/path"), "HTTPS默认端口应为443");
  }
  ...
  void testExtractPortFromRelativeUrl() {
    assertEquals(80, HttpProxyVerticle.extractPortFromUrl("/api/test"), "相对URL应返回HTTP默认端口80");
  }
  ...
  void testExtractPortFromInvalidUrl() {
    assertEquals(-1, HttpProxyVerticle.extractPortFromUrl("://invalid-url"), "无效URL应返回-1");
  }
  ...
  void testExtractPortFromEmptyUrl() {
    assertEquals(-1, HttpProxyVerticle.extractPortFromUrl(""), "空URL应返回-1");
    assertEquals(-1, HttpProxyVerticle.extractPortFromUrl("   "), "空白URL应返回-1");
  }
  ...
  void testExtractPortFromNonStandardPort() {
    assertEquals(
        3000, HttpProxyVerticle.extractPortFromUrl("http://localhost:3000/api"), "应该提取到端口3000");
    assertEquals(
        9443,
        HttpProxyVerticle.extractPortFromUrl("https://secure.example.com:9443/admin"),
        "应该提取到端口9443");
  }
  ...
  void testDeployWithPreProxy(Vertx vertx, VertxTestContext testContext) {
    ...
    JsonObject config =
        new JsonObject()
            .put("proxy-server", new JsonObject().put("port", PROXY_PORT + 2))
            .put(
                "proxy-pre",
                new JsonObject().put("ip", "192.168.1.1").put("port", 8080).put("type", "HTTP"));
  ...
  void testMultipleInstancesDeploy(Vertx vertx, VertxTestContext testContext) {
    ...
    vertx.sharedData().getLocalMap(ConfigConstant.LOCAL).put(ConfigConstant.GLOBAL_CONFIG, config1);
            ...
            id1 -> {
              ...
              return vertx
                  ...
                  .compose(
                      id2 -> {
                        // 清理
                        return vertx.undeploy(id1).compose(v -> vertx.undeploy(id2));
                      });
            })
        .onComplete(
            testContext.succeeding(
  ...
  void testMissingProxyPreConfig(Vertx vertx, VertxTestContext testContext) {
    JsonObject config =
        new JsonObject().put("proxy-server", new JsonObject().put("port", PROXY_PORT + 4));
    // 完全没有proxy-pre键
