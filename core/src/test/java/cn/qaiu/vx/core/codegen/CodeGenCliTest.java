    ...
    void testGenerateEntity(VertxTestContext testContext) {
      testContext.verify(
          () -> {
            ...
            CodeGenCli.CodeGenResult result =
                codeGenCli.generateEntity(tableName, className, packageName, outputPath);
    ...
    void testGenerateDao(VertxTestContext testContext) {
      testContext.verify(
          () -> {
            ...
            CodeGenCli.CodeGenResult result =
                codeGenCli.generateDao(tableName, className, packageName, outputPath);
            ...
            assertTrue(
                content.contains("public Future<Optional<Product>> findById"), "应该包含findById方法");
    ...
    void testGenerateController(VertxTestContext testContext) {
      testContext.verify(
          () -> {
            ...
            CodeGenCli.CodeGenResult result =
                codeGenCli.generateController(tableName, className, packageName, outputPath);
    ...
    void testGenerateAll(VertxTestContext testContext) {
      testContext.verify(
          () -> {
            ...
            CodeGenCli.CodeGenResult result =
                codeGenCli.generateAll(tableName, className, packageName, outputPath);
  ...
  class EdgeCaseTest {
    ...
    void testEmptyParameters(VertxTestContext testContext) {
      testContext.verify(
          () -> {
            // 测试空表名
            CodeGenCli.CodeGenResult result1 =
                codeGenCli.generateEntity("", "User", "com.example", "target/generated");
            assertFalse(result1.isSuccess(), "空表名应该失败");

            // 测试空类名
            CodeGenCli.CodeGenResult result2 =
                codeGenCli.generateEntity("user", "", "com.example", "target/generated");
            assertFalse(result2.isSuccess(), "空类名应该失败");

            // 测试空包名
            CodeGenCli.CodeGenResult result3 =
                codeGenCli.generateEntity("user", "User", "", "target/generated");
