...
/**
 * 高并发内存性能测试 测试框架在高并发场景下的内存占用和增长率
 *
 * @author QAIU
 */
...
@Disabled(
    "性能测试在常规测试中禁用，需要手动运行: mvn test -Dtest=HighConcurrencyMemoryTest -DskipPerformanceDisable=true")
...
class HighConcurrencyMemoryTest {
  ...
  static void setUpServer(Vertx vertx, VertxTestContext testContext) {
    HttpClientOptions options = new HttpClientOptions().setMaxPoolSize(200).setKeepAlive(true);

    client = vertx.createHttpClient(options);

    vertx
        .deployVerticle(new MemoryTestServerVerticle())
        .onSuccess(
            id -> {
              deploymentId = id;
  ...
  void testConcurrentRequestMemoryGrowth(Vertx vertx, VertxTestContext testContext) {
    ...
    for (int i = 0; i < totalRequests; i++) {
      client
          .request(HttpMethod.GET, PORT, "localhost", "/api/data")
          ...
          .onComplete(
              ar -> {
                ...
                if (completed == totalRequests) {
                  ...
                  long peakMemory =
                      memorySnapshots.stream().mapToLong(Long::longValue).max().orElse(0);
  ...
  void testMemoryStabilityUnderLoad(Vertx vertx, VertxTestContext testContext) {
    ...
    vertx.setTimer(
        ...
        id -> {
          ...
          System.out.println(
              "吞吐量: " + (successCount.get() / Math.max(1, testDurationSeconds)) + " req/s");
          System.out.println("初始内存: " + formatBytes(initialMemory));
          System.out.println("最终内存: " + formatBytes(finalMemory));
          ...
          if (!memorySnapshots.isEmpty()) {
            ...
            assertTrue(
                memoryVariation < initialMemory * 0.5,
  ...
  void testLargeResponseMemory(VertxTestContext testContext) {
    ...
    for (int i = 0; i < totalRequests; i++) {
      client
          .request(HttpMethod.GET, PORT, "localhost", "/api/large")
          .compose(req -> req.send().compose(resp -> resp.body()))
          .onComplete(
              ar -> {
