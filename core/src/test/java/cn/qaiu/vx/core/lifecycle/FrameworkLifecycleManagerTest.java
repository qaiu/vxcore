...
public class FrameworkLifecycleManagerTest {
  ...
  void testInitialState() {
    assertEquals(
        FrameworkLifecycleManager.LifecycleState.INITIAL,
                  ...
                  .onSuccess(
                      v2 -> {
                        testContext.verify(
                            () -> {
                              assertEquals(
                                  FrameworkLifecycleManager.LifecycleState.STOPPED,
                                  lifecycleManager.getState(),
                                  "停止后状态应该是STOPPED");
                              testContext.completeNow();
                            });
                  ...
                  .onFailure(
                      error -> {
                        testContext.verify(
                            () -> {
                              assertTrue(
                                  error.getMessage().contains("already starting or started"),
                                  "应该返回重复启动的错误信息");
                              testContext.completeNow();
                            });
                      });
            ...
            error -> {
              testContext.verify(
                  () -> {
                    ...
                    assertEquals(
                        FrameworkLifecycleManager.LifecycleState.FAILED,
                        lifecycleManager.getState(),
                        "失败后状态应该是FAILED");
                    testContext.completeNow();
                  });
