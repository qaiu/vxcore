...
public class JwtAuthProviderTest {
  ...
  void testGenerateRefreshToken(VertxTestContext testContext) throws Exception {
    ...
    provider
        ...
        .compose(
            v -> {
              JsonObject claims =
                  new JsonObject().put("sub", "user456").put("username", "refreshuser");
  ...
  void testRevokeToken(VertxTestContext testContext) throws Exception {
    ...
    provider
        ...
        .compose(
            v -> {
              JsonObject claims = new JsonObject().put("sub", "userToRevoke");
        ...
        .onComplete(
            testContext.failing(
                e -> {
                  assertTrue(
                      e.getMessage().contains("revoked") || e.getMessage().contains("blacklist"),
                      "撤销后的Token验证应该失败");
                  testContext.completeNow();
                }));
  }
  ...
  void testRevokeTokenOnly(VertxTestContext testContext) throws Exception {
    ...
    provider
        ...
        .compose(
            v -> {
              JsonObject claims = new JsonObject().put("sub", "userRevokeOnly");
  ...
  void testGenerateRandomSecret() throws Exception {
    JsonObject configJson = new JsonObject().put("jwt-algorithm", "HS256");
  ...
  void testGenerateRSAKeyPair() throws Exception {
    JsonObject configJson = new JsonObject().put("jwt-algorithm", "RS256");
  ...
  void testIsRefreshTokenNormal(VertxTestContext testContext) throws Exception {
    ...
    provider
        ...
        .compose(
            v -> {
              JsonObject claims = new JsonObject().put("sub", "normalUser");
