...
/**
 * VXCoreApplication核心应用测试类 测试应用启动、停止、状态管理等核心功能
 *
 * <p>注意: 这些测试需要完整的配置文件,属于集成测试范畴。 CI 环境下自动跳过；本地可手动运行。
 *
...
public class VXCoreApplicationTest {
  ...
  void setUp(Vertx vertx) throws Exception {
    ...
    vertx
        .fileSystem()
        .writeFileBlocking(
            configPath, io.vertx.core.buffer.Buffer.buffer(testConfig.encodePrettily()));
  }

  @AfterEach
  void tearDown(VertxTestContext testContext) {
    if (application != null && application.isStarted()) {
      application.stop().onComplete(ar -> testContext.completeNow());
    } else {
  ...
  void testStaticRunMethod() {
    ...
    assertDoesNotThrow(
        () -> {
          VXCoreApplication.class.getDeclaredMethod(
              "run", String[].class, io.vertx.core.Handler.class);
        },
        "静态run方法应该存在");
  }
  ...
  void testMultipleInstances() {
    ...
    assertSame(app1.getLifecycleManager(), app2.getLifecycleManager(), "多个应用实例共享同一个生命周期管理器");
  }
