...
class CodeGeneratorTest {
  ...
  class CodeGenerationTest {
    ...
    void testGenerateToString(VertxTestContext testContext) {
      ...
      codeGenerator
          .generateToString("entity.ftl", context)
          .onSuccess(
              result -> {
                testContext.verify(
                    () -> {
    ...
    void testGenerateToFile(VertxTestContext testContext) {
      ...
      codeGenerator
          .generate("entity.ftl", context, outputPath)
          .onSuccess(
              result -> {
                testContext.verify(
                    () -> {
    ...
    void testBatchGeneration(VertxTestContext testContext) {
      ...
      TemplateConfig[] templates = {
        ...
        new TemplateConfig(
            "controller.ftl", "target/generated/com/example/controller/OrderController.java")
      };

      codeGenerator
          .generateBatch(templates, context)
          .onSuccess(
              results -> {
                testContext.verify(
                    () -> {
    ...
    void testConfigValidation(VertxTestContext testContext) {
      testContext.verify(
          () -> {
            ...
            GeneratorConfig invalidConfig2 =
                new GeneratorConfig().setOutputPath("").setPackageName("com.example");
            assertFalse(codeGenerator.validateConfig(invalidConfig2), "空输出路径应该验证失败");

            GeneratorConfig invalidConfig3 =
                new GeneratorConfig().setOutputPath("target/generated").setPackageName("");
    ...
    void testConfigProperties(VertxTestContext testContext) {
      testContext.verify(
          () -> {
            ...
            assertEquals(
                "default", config.getCustomPropertyAsString("nonexistent", "default"), "默认值应该正确");
  ...
  class ErrorHandlingTest {
    ...
    void testInvalidTemplateHandling(VertxTestContext testContext) {
      ...
      codeGenerator
          .generateToString("nonexistent.ftl", context)
          .onSuccess(
              result -> {
    ...
    void testInvalidOutputPathHandling(VertxTestContext testContext) {
      ...
      codeGenerator
          .generate("entity.ftl", context, invalidPath)
          .onSuccess(
              result -> {
                testContext.failNow(new AssertionError("应该失败但成功了"));
              })
