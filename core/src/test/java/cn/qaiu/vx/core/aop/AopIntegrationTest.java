...
/**
 * AOP 框架集成测试 测试完整的 AOP 流程：切面注册 -> 方法拦截 -> 通知执行
 *
...
public class AopIntegrationTest {
  ...
  void testBeforeAdviceExecution() throws Throwable {
    ...
    assertTrue(
        executionLog.indexOf("before") < executionLog.indexOf("target"), "before 应该在 target 之前执行");
  }
  ...
  void testAfterAdviceExecution() throws Throwable {
    ...
    assertTrue(
        executionLog.indexOf("target") < executionLog.indexOf("after"), "after 应该在 target 之后执行");
  }
  ...
  void testAfterThrowingCatchesException() throws Throwable {
    ...
    assertTrue(executionLog.contains("afterThrowing:Test exception"), "应该捕获并处理异常");
  }
  ...
  static class AfterThrowingTestAspect {
    @AfterThrowing(
        value = "execution(* cn.qaiu.vx.core.aop.AopIntegrationTest.UserService.*(..))",
        throwing = "ex")
