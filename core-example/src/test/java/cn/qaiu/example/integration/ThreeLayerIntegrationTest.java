...
/**
 * 三层架构集成测试 测试Controller -> Service -> DAO 完整流程 注意：此测试需要启动HTTP服务器，在CI环境下可能因端口冲突而被禁用
 *
...
public class ThreeLayerIntegrationTest {
    ...
    userService
        ...
        .onFailure(
            error -> {
              testContext.verify(
                  () -> {
                    assertTrue(error.getMessage().contains("用户名不能为空"), "应该返回用户名不能为空的错误");
              ...
              userService
                  ...
                  .onFailure(
                      error2 -> {
                        testContext.verify(
                            () -> {
                              assertTrue(error2.getMessage().contains("邮箱不能为空"), "应该返回邮箱不能为空的错误");
                  ...
                  .onFailure(
                      error -> {
                        testContext.verify(
                            () -> {
                              LOGGER.info(
                                  "Error handling test - Invalid user creation failed as expected: {}",
                                  error.getMessage());
                              testContext.completeNow();
                            });
                      });
  ...
  void testConcurrentOperations(VertxTestContext testContext) {
    // 并发创建多个用户
    List<Future<JsonResult<User>>> futures =
        List.of(
