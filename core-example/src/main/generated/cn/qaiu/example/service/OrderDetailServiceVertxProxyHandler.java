...
import cn.qaiu.example.service.OrderWithDetailsAndUser;
import Object;
import Long;
...
public class OrderDetailServiceVertxProxyHandler extends ProxyHandler {
  ...
  public OrderDetailServiceVertxProxyHandler(Vertx vertx, OrderDetailService service){
    this(vertx, service, DEFAULT_CONNECTION_TIMEOUT);
  }

  public OrderDetailServiceVertxProxyHandler(Vertx vertx, OrderDetailService service, long timeoutInSecond){
    this(vertx, service, true, timeoutInSecond);
  }

  public OrderDetailServiceVertxProxyHandler(Vertx vertx, OrderDetailService service, boolean topLevel, long timeoutInSecond){
    this(vertx, service, true, timeoutInSecond, false);
  }

  public OrderDetailServiceVertxProxyHandler(Vertx vertx, OrderDetailService service, boolean topLevel, long timeoutSeconds, boolean includeDebugInfo) {
      this.vertx = vertx;
  ...
  public void handle(Message<JsonObject> msg) {
    try{
      ...
      accessed();
      switch (action) {
        case "getOrderWithDetailsAndUser": {
          service.getOrderWithDetailsAndUser(json.getValue("orderId") == null ? null : (json.getLong("orderId").longValue())).onComplete(HelperUtils.createHandler(msg, includeDebugInfo));
          break;
        }
        case "getUserConsumptionAnalysis": {
          service.getUserConsumptionAnalysis(json.getValue("userId") == null ? null : (json.getLong("userId").longValue())).onComplete(HelperUtils.createHandler(msg, includeDebugInfo));
          break;
        }
        case "getById": {
