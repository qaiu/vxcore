...
public class AuthController {
  ...
  // 模拟用户数据库
  private static final Map<String, JsonObject> USERS =
  ...
  private JwtAuthProvider getJwtAuthProvider() {
    if (jwtAuthProvider == null) {
      ...
      JsonObject securityConfig =
          globalConfig != null ? globalConfig.getJsonObject("security") : null;
      ...
      return provider
          ...
          .compose(
              user -> {
                ...
                LOGGER.info(
                    "Token refreshed successfully for user: {}", principal.getString("sub"));
                return Future.succeededFuture(JsonResult.data(result));
              })
          .recover(
              err -> {
