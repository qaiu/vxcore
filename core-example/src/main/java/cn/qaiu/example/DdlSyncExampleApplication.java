...
/**
 * 带自动DDL同步的示例应用 演示如何使用 @EnableDdlSync 注解启用自动ORM功能
 *
 * <p>配置说明：
 *
 * <ul>
 *   <li>strategy: DDL同步策略
 *       <ul>
 *         <li>AUTO - 自动检测并同步表结构（默认）
 *         <li>CREATE - 只创建表，不更新
 *         <li>UPDATE - 只更新表，不创建
 *         <li>VALIDATE - 只验证不执行
 *         <li>NONE - 禁用DDL同步
 *       </ul>
 *   <li>entityPackages - 实体类包路径
 *   <li>showDdl - 是否在日志中显示DDL语句
 *   <li>autoExecute - 是否自动执行DDL
 * </ul>
 *
...
public class DdlSyncExampleApplication {
  ...
  private static void onStartup(JsonObject config) {
    ...
    log.info(
        "  - Gateway prefix: {}",
        config.getJsonObject("custom", new JsonObject()).getString("gatewayPrefix", "/"));
