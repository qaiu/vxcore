...
/**
 * 用户数据访问对象 演示三层架构中的DAO层 使用内存存储模拟数据库操作，继承LambdaDao获得lambda查询功能
 *
...
public class UserDao extends LambdaDao<User, Long> {
  ...
  /** 重写getExecutor方法，避免父类尝试初始化数据库连接 UserDao使用内存存储，不需要数据库连接 */
  @Override
  protected cn.qaiu.db.dsl.core.JooqExecutor getExecutor() {
    // UserDao使用内存存储，不需要数据库连接
    throw new UnsupportedOperationException(
        "UserDao uses mock storage, no database connection needed");
  }
  ...
  public Future<User> save(User user) {
    return insert(user).map(optional -> optional.orElse(null));
  }
  ...
  public Future<List<User>> findByAgeRange(int minAge, int maxAge) {
    ...
    List<User> users =
        userStorage.values().stream()
            .filter(
                user -> user.getAge() != null && user.getAge() >= minAge && user.getAge() <= maxAge)
            .toList();
  ...
  public Future<List<User>> findByMinBalance(java.math.BigDecimal minBalance) {
    ...
    List<User> users =
        userStorage.values().stream()
            .filter(
                user -> user.getBalance() != null && user.getBalance().compareTo(minBalance) >= 0)
            .toList();
