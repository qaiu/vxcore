...
public class OrderDao extends LambdaDao<Order, Long> {
  ...
  public Future<Order> createOrder(
      String orderNo, Long userId, Long productId, Integer quantity, BigDecimal unitPrice) {
  ...
  public Future<List<Order>> findByUserId(Long userId) {
    ...
    return lambdaList(lambdaQuery().eq(Order::getUserId, userId));
  }
  ...
  public Future<List<Order>> findByAmountRange(BigDecimal minAmount, BigDecimal maxAmount) {
    ...
    return lambdaList(lambdaQuery().between(Order::getTotalAmount, minAmount, maxAmount));
  }
  ...
  public Future<List<Order>> searchOrders(String keyword) {
    ...
    return lambdaList(lambdaQuery().like(Order::getOrderNo, "%" + keyword + "%"));
  }
        ...
        .map(
            allOrders -> {
              ...
              for (Order.OrderStatus status : Order.OrderStatus.values()) {
                long count =
                    allOrders.stream().filter(order -> order.getStatus() == status).count();
  ...
  /** 根据时间范围查找订单 */
  public Future<List<Order>> findByTimeRange(
        ...
        .map(
            allOrders ->
                allOrders.stream()
                    .filter(
                        order ->
                            order.getCreateTime().isAfter(startTime)
                                && order.getCreateTime().isBefore(endTime))
