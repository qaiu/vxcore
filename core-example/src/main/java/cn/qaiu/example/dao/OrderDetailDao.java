...
/**
 * 订单详情数据访问对象 (DAO) - 演示 MyBatis-Plus 风格的 Lambda 查询 支持 DI 注入，框架自动管理 JooqExecutor
 *
...
public class OrderDetailDao extends LambdaDao<OrderDetail, Long> {
  ...
  public Future<List<OrderDetail>> findByOrderId(Long orderId) {
    return lambdaList(lambdaQuery().eq(OrderDetail::getOrderId, orderId));
  }

  /** 根据商品ID查询订单详情列表 */
  public Future<List<OrderDetail>> findByProductId(Long productId) {
    return lambdaList(lambdaQuery().eq(OrderDetail::getProductId, productId));
  }

  /** 根据分类查询订单详情列表 */
  public Future<List<OrderDetail>> findByCategory(String category) {
    return lambdaList(lambdaQuery().eq(OrderDetail::getCategory, category));
  }

  /** 根据订单ID删除订单详情 */
  public Future<Integer> deleteByOrderId(Long orderId) {
    return lambdaDelete(lambdaQuery().eq(OrderDetail::getOrderId, orderId));
  }

  /** 根据商品ID删除订单详情 */
  public Future<Integer> deleteByProductId(Long productId) {
    return lambdaDelete(lambdaQuery().eq(OrderDetail::getProductId, productId));
  }

  /** 统计订单详情数量 */
  public Future<Long> countByOrderId(Long orderId) {
    return lambdaCount(lambdaQuery().eq(OrderDetail::getOrderId, orderId));
  }

  /** 统计商品销售数量 */
  public Future<Long> countByProductId(Long productId) {
    return lambdaCount(lambdaQuery().eq(OrderDetail::getProductId, productId));
  }
  ...
  public Future<Integer> updateCategoryByOrderId(Long orderId, String newCategory) {
    ...
    return lambdaUpdate(lambdaQuery().eq(OrderDetail::getOrderId, orderId), updateEntity);
  }
  ...
  public Future<Integer> updatePriceByProductId(Long productId, BigDecimal newPrice) {
    ...
    return lambdaUpdate(lambdaQuery().eq(OrderDetail::getProductId, productId), updateEntity);
  }
    ...
    return executor
        .executeQuery(
            DSL.select(
                ...
                .join(DSL.table("orders").as("o"))
                .on(DSL.field("od.order_id", Long.class).eq(DSL.field("o.id", Long.class)))
                .join(DSL.table("dsl_user").as("u"))
                .on(DSL.field("o.user_id", Long.class).eq(DSL.field("u.id", Long.class)))
    ...
    return executor
        .executeQuery(
            DSL.select(
                ...
                .leftJoin(DSL.table("orders").as("o"))
                .on(DSL.field("u.id", Long.class).eq(DSL.field("o.user_id", Long.class)))
                .leftJoin(DSL.table("order_details").as("od"))
                .on(DSL.field("o.id", Long.class).eq(DSL.field("od.order_id", Long.class)))
