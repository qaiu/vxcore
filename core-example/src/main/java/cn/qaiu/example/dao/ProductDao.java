 ...
 *
 * <p>继承 LambdaDao，父类已提供基础 CRUD 功能： - insert(T entity) - update(T entity) - delete(ID id) -
 * findById(ID id) - findAll() - findByCondition(Condition condition) - count() - lambdaQuery()
 *
 * <p>本类只提供个性化的业务查询方法。
 *
 * @author QAIU
...
public class ProductDao extends LambdaDao<Product, Long> {
  ...
  /** 创建产品 */
  public Future<Product> createProduct(
      String name, String category, BigDecimal price, Integer stock, String description) {
  ...
  public Future<List<Product>> findByCategory(String category) {
    ...
    return lambdaList(lambdaQuery().eq(Product::getCategory, category));
  }
  ...
  public Future<List<Product>> findByStatus(Product.ProductStatus status) {
    ...
    return lambdaList(lambdaQuery().eq(Product::getStatus, status));
  }
  ...
  public Future<List<Product>> findByPriceRange(BigDecimal minPrice, BigDecimal maxPrice) {
    ...
    return lambdaList(lambdaQuery().between(Product::getPrice, minPrice, maxPrice));
  }
  ...
  public Future<List<Product>> findByStockRange(Integer minStock, Integer maxStock) {
    ...
    return lambdaList(lambdaQuery().between(Product::getStock, minStock, maxStock));
  }
  ...
  public Future<List<Product>> searchProducts(String keyword) {
    ...
    return lambdaList(lambdaQuery().like(Product::getName, "%" + keyword + "%"));
  }
        ...
        .map(
            allProducts -> {
              ...
              for (Product.ProductStatus status : Product.ProductStatus.values()) {
                long count =
                    allProducts.stream().filter(product -> product.getStatus() == status).count();
  ...
  public Future<Long> countByCategory(String category) {
    ...
    return lambdaCount(lambdaQuery().eq(Product::getCategory, category));
  }
  ...
  public Future<Long> countByStatus(Product.ProductStatus status) {
    ...
    return lambdaCount(lambdaQuery().eq(Product::getStatus, status));
  }
  ...
  public Future<List<Product>> findOutOfStockProducts() {
    ...
    return lambdaList(lambdaQuery().eq(Product::getStatus, Product.ProductStatus.OUT_OF_STOCK));
  }
  ...
  public Future<List<Product>> findByNameLike(String namePattern) {
    ...
    return lambdaList(lambdaQuery().like(Product::getName, "%" + namePattern + "%"));
  }
  ...
  /** 根据时间范围查找产品 */
  public Future<List<Product>> findByTimeRange(
        ...
        .map(
            allProducts ->
                allProducts.stream()
                    .filter(
                        product ->
                            product.getCreateTime().isAfter(startTime)
                                && product.getCreateTime().isBefore(endTime))
  ...
  public Future<List<JsonObject>> getProductsWithOrders() {
    ...
    return lambdaList(
            lambdaQuery()
                .leftJoin(
                    cn.qaiu.example.entity.Order.class,
                    (product, order) -> DSL.field("product_id", Long.class).eq(product.getId())))
        .map(
            products ->
                products.stream()
                    .map(Product::toJson)
  ...
  public Future<List<JsonObject>> getProductsByUserId(Long userId) {
    ...
    Query query =
        DSL.select(productIdField, productNameField, productCategoryField, productPriceField)
  ...
  public Future<JsonObject> getProductWithUserStats(Long productId) {
    ...
    Field<BigDecimal> totalAmountField =
        DSL.sum(DSL.field("o.total_amount", BigDecimal.class)).as("total_amount");
