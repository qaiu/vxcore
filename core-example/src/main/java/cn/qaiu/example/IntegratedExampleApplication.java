...
/**
 * 集成示例应用 演示如何将core-database模块的实现注入到core模块中 避免循环依赖问题
 *
 * @author QAIU
 */
public class IntegratedExampleApplication extends AbstractVerticle {
  ...
  /** 注入数据库实现 将core-database模块的DataSourceManager实现注入到core模块的DataSourceComponent中 */
  private io.vertx.core.Future<Void> injectDatabaseImplementation() {
    return io.vertx.core.Future.future(
        promise -> {
          try {
            // 获取框架生命周期管理器
            ...
            cn.qaiu.db.datasource.DataSourceManager databaseManager =
  ...
  /** 初始化数据源 */
  private io.vertx.core.Future<Void> initializeDataSources(
      cn.qaiu.db.datasource.DataSourceManager manager) {
    // 创建H2内存数据库配置
    DataSourceConfig h2Config =
    ...
    // 注册并初始化数据源
    return manager
        .registerDataSource("default", h2Config)
        .compose(
            v -> {
  ...
  public void stop(Promise<Void> stopPromise) {
    ...
    // 停止框架
    FrameworkLifecycleManager.getInstance()
        .stop()
        .onSuccess(
            v -> {
