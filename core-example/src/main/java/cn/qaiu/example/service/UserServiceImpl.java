...
/**
 * 用户服务实现类 演示 VXCore 框架的服务层，支持自动SQL执行器创建
 *
...
public class UserServiceImpl extends JServiceImpl<User, Long> implements UserService {
        ...
        .compose(
            optional -> {
              if (optional.isPresent()) {
                ...
                return updateById(user).map(result -> result.isPresent());
              } else {
  ...
  public Future<List<User>> getUsersByAgeRange(Integer minAge, Integer maxAge) {
    ...
    return lambdaList(
        lambdaQuery().between(User::getAge, minAge, maxAge).orderByDesc(User::getCreateTime));
  }
  ...
  public Future<Boolean> existsByEmail(String email) {
    ...
    return lambdaCount(lambdaQuery().eq(User::getEmail, email)).map(count -> count > 0);
  }
