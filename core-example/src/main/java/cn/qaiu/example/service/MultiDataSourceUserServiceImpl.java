...
/**
 * 多数据源用户服务实现类 演示多数据源的使用，支持 DI 注入
 *
 * @author <a href="https://qaiu.top">QAIU</a>
 */
@Singleton
@DataSource("user") // 指定使用 user 数据源
public class MultiDataSourceUserServiceImpl extends JServiceImpl<User, Long>
    implements UserService {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(MultiDataSourceUserServiceImpl.class);
  ...
  /**
   * 构造函数 - 使用 DI 注入 JooqExecutor 根据 @DataSource 注解自动选择对应的数据源
   *
  ...
  public MultiDataSourceUserServiceImpl(JooqExecutor executor) {
    ...
    LOGGER.info(
        "MultiDataSourceUserServiceImpl initialized with DI injection for 'user' datasource");
  }
  ...
  public Future<List<User>> findActiveUsers() {
    ...
    return lambdaList(
        lambdaQuery().eq(User::getStatus, User.UserStatus.ACTIVE).orderByDesc(User::getCreateTime));
  }
  ...
  public Future<List<User>> searchByName(String keyword) {
    ...
    return lambdaList(
        lambdaQuery().like(User::getUsername, keyword).orderByDesc(User::getCreateTime).limit(20));
  }
  ...
  public Future<Boolean> updateUserBalance(Long userId, String balance) {
    ...
    return lambdaUpdate(lambdaQuery().eq(User::getId, userId), user).map(rows -> rows > 0);
  }
  ...
  /** 演示方法级别的数据源切换 这个方法会使用 backup 数据源 */
  @DataSource("backup")
  ...
  /** 演示方法级别的数据源切换 这个方法会使用 archive 数据源 */
  @DataSource("archive")
  ...
  public Future<List<User>> findUsersByName(String name) {
    ...
    return lambdaList(lambdaQuery().eq(User::getUsername, name).orderByDesc(User::getCreateTime));
  }
  ...
  public Future<User> registerUser(UserRegistrationRequest request) {
    ...
    if (!PASSWORD_PATTERN.matcher(request.getPassword()).matches()) {
      return Future.failedFuture(new IllegalArgumentException("密码必须至少8个字符,包含至少1个字母和1个数字"));
    }
