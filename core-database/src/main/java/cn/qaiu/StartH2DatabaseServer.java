...
/**
 * 启动H2数据库服务器并初始化连接池 h2服务启动后 不会立即创建数据库文件，只有在连接时才会创建 <br>
 * 可通过 -tcpAllowOthers 允许远程连接（注意安全风险 ） <br>
 * 可通过 -ifNotExists 避免重复启动时报错 <br>
 * 可通过 -tcpPort 指定端口，默认9092 <br>
 * 启动后可通过 JDBC URL 连接：jdbc:h2:tcp://localhost:9092/~/test <br>
 * 数据库名需要带路径，~/test 表示用户目录下的 test 数据库 <br>
 * Create date 2024-06-10 10:00:00
 *
        ...
        .compose(
            config -> {
              ...
              PoolOptions poolOptions =
                  new PoolOptions().setMaxSize(config.getInteger("maxPoolSize", 5));
              ...
              Runtime.getRuntime()
                  .addShutdownHook(
                      new Thread(
                          () -> {
                            System.out.println(
                                "Shutdown hook: closing DB pool and H2 server and Vertx...");
                            try {
                              DbPoolHolder.close();
