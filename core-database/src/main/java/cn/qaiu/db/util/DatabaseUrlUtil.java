    ...
    try {
      ...
      try {
        ...
        java.sql.Connection jdbcConn =
            (java.sql.Connection) unwrapMethod.invoke(vertxConn, java.sql.Connection.class);
      ...
      try {
        ...
        if (connField != null) {
          ...
          if (connectionImpl != null) {
            ...
            Method getJdbcConnectionMethod =
                findMethod(connectionImpl.getClass(), "getJDBCConnection");
            if (getJdbcConnectionMethod != null) {
              getJdbcConnectionMethod.setAccessible(true);
              @SuppressWarnings("resource") // 不关闭连接，因为它是从 Vert.x 连接池借用的
              java.sql.Connection jdbcConn =
                  (java.sql.Connection) getJdbcConnectionMethod.invoke(connectionImpl);
              DatabaseMetaData metaData = jdbcConn.getMetaData();
              return metaData.getURL();
            }
          }
