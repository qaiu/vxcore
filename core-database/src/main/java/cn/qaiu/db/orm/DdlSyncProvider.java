...
/**
 * DDL同步提供者实现
 * 实现OrmSyncProvider SPI接口，供core模块在启动时调用
 *
...
public class DdlSyncProvider implements OrmSyncProvider {
     ...
     * 根据策略执行同步
     */
    private Future<OrmSyncResult> executeSyncByStrategy(Pool pool, Set<Class<?>> entityClasses,
                                                         DdlSyncStrategy strategy, boolean showDdl) {
        Promise<OrmSyncResult> promise = Promise.promise();
     ...
     * 执行自动同步（创建+更新）
     */
    private Future<OrmSyncResult> executeAutoSync(Pool pool, Set<Class<?>> entityClasses, boolean showDdl) {
     ...
     * 执行仅创建
     */
    private Future<OrmSyncResult> executeCreateOnly(Pool pool, Set<Class<?>> entityClasses, boolean showDdl) {
     ...
     * 执行仅更新
     */
    private Future<OrmSyncResult> executeUpdateOnly(Pool pool, Set<Class<?>> entityClasses, boolean showDdl) {
    ...
    private Future<OrmSyncResult> executeValidateOnly(Pool pool, Set<Class<?>> entityClasses) {
        ...
        return EnhancedCreateTable.hasTablesNeedingSync(pool)
                .map(needsSync -> {
                    if (needsSync) {
                        return OrmSyncResult.failure("Schema validation failed: tables need synchronization");
                    }
