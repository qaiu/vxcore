 ...
 * 增强的DAO基类
 *
 * <p>在基础CRUD功能基础上，增加了以下高级查询功能： - 分页查询：支持分页参数和结果封装 - 条件查询：支持复杂查询条件构建 - 排序查询：支持单字段和多字段排序 -
 * 聚合查询：支持统计、分组等操作 - 批量操作：支持批量插入、更新、删除
 *
 * <p>专为Vert.x异步服务设计，所有分页和查询对象都支持序列化
 *
 * @param <T> 实体类型
 * @param <ID> 主键类型
...
public abstract class EnhancedDao<T, ID> implements JooqDao<T, ID> {
  ...
  public EnhancedDao(JooqExecutor executor, Class<T> entityClass) {
    ...
    LOGGER.debug(
        "EnhancedDao initialized with manual JooqExecutor for entity: {}",
  ...
  public EnhancedDao(Class<T> entityClass) {
    ...
    LOGGER.debug(
        "EnhancedDao initialized with auto-executor mode for entity: {} with datasource: {}",
        entityClass.getSimpleName(),
        dataSourceName);
  }

  /** 构造函数3：无参构造函数（最推荐方式） 自动通过泛型获取实体类类型 */
  ...
  public EnhancedDao() {
    ...
    LOGGER.debug(
        "EnhancedDao initialized with auto-executor mode for entity: {} (from generics) with datasource: {}",
        entityClass.getSimpleName(),
        dataSourceName);
  }
  ...
  public Future<Boolean> exists(ID id) {
    return count(DSL.field(primaryKeyField).eq(id)).map(count -> count > 0);
  }
  ...
  public Future<PageResult> findPage(PageRequest pageRequest, Condition condition) {
    try {
      ...
      Future<List<T>> dataFuture =
          getExecutor().executeQuery(selectQuery).map(entityMapper::fromMultiple);

      return countFuture.compose(
          totalRecords -> {
            return dataFuture.map(
                entities -> {
                  // 转换为JsonObject列表
                  List<JsonObject> jsonEntities =
                      entities.stream()
                          .map(
                              entity -> {
  ...
  public Future<PageResult> findPage(PageRequest pageRequest, List<QueryCondition> conditions) {
    try {
      Condition condition =
          convertQueryConditionsToJooq(conditions, QueryCondition.ConditionType.AND);
      return findPage(pageRequest, condition);
    } catch (Exception e) {
  ...
  public Future<List<T>> batchInsert(List<T> entities) {
    ...
    try {
      List<Future<Optional<T>>> insertFutures =
          entities.stream().map(this::insert).collect(Collectors.toList());
  ...
  public Future<List<T>> batchUpdate(List<T> entities) {
    ...
    try {
      List<Future<Optional<T>>> updateFutures =
          entities.stream().map(this::update).collect(Collectors.toList());
  ...
  public Future<Integer> batchDelete(List<ID> ids) {
    ...
    try {
      List<Future<Boolean>> deleteFutures =
          ids.stream().map(this::delete).collect(Collectors.toList());
  ...
  /** 构建分页查询SQL */
  private Query buildPageableSelect(
      String tableName, Condition condition, PageRequest pageRequest) {
    ...
    if (pageRequest.hasSorting()) {
      sql +=
          String.format(
              " ORDER BY %s %s", pageRequest.getSortField(), pageRequest.getSortDirection());
    } else {
      sql += String.format(" ORDER BY %s ASC", primaryKeyField);
    }

    // 添加LIMIT和OFFSET
    sql += String.format(" LIMIT %d OFFSET %d", pageRequest.getLimit(), pageRequest.getOffset());

    return DSL.query(sql);
  }

  /** 构建排序查询SQL */
  private Query buildOrderedSelect(
      String tableName, Condition condition, String sortField, boolean ascending, Integer limit) {
    String sql = String.format("SELECT * FROM %s", tableName);
  ...
  /** 构建多字段排序查询SQL */
  private Query buildMultiOrderedSelect(
      String tableName, Condition condition, Map<String, Boolean> sortFields, Integer limit) {
    String sql = String.format("SELECT * FROM %s", tableName);
  ...
  protected Condition convertQueryConditionsToJooq(
      List<QueryCondition> conditions, QueryCondition.ConditionType connective) {
          ...
          .map(
              generatedId -> {
                ...
                if (generatedId != null && generatedId > 0) {
                  ...
                  LOGGER.warn(
                      "Insert returned invalid ID: {}, returning empty Optional", generatedId);
                  return Optional.empty();
                }
              });
    } catch (Exception e) {
  ...
  private Future<Optional<T>> performFindById(ID id) {
    ...
    try {
      ...
      return getExecutor().executeQuery(selectQuery).map(rowSet -> entityMapper.fromSingle(rowSet));
    } catch (Exception e) {
  ...
  private Future<List<T>> performFindByCondition(Condition condition) {
    try {
      ...
      return getExecutor().executeQuery(selectQuery).map(entityMapper::fromMultiple);
    } catch (Exception e) {
    ...
    try {
      ...
      if (executor == null) {
        throw new IllegalStateException(
            "No JooqExecutor available for datasource: " + dataSourceName);
      }

      LOGGER.info(
          "JooqExecutor auto-initialized for datasource: {} in EnhancedDao: {}",
          dataSourceName,
    ...
    try {
      ...
      try {
        ...
        Class<? extends java.lang.annotation.Annotation> tableAnnotationClass =
            (Class<? extends java.lang.annotation.Annotation>)
                Class.forName("cn.qaiu.db.dsl.Table");
  ...
  /** 切换到指定数据源执行操作 */
  protected <R> Future<R> executeWithDataSource(
      String targetDataSource, java.util.function.Function<JooqExecutor, Future<R>> operation) {
    if (!autoExecutorMode) {
    ...
    return Future.future(
        promise -> {
          ...
          try {
            ...
            operation.apply(targetExecutor).onSuccess(promise::complete).onFailure(promise::fail);
          } finally {
            // 恢复原始数据源
