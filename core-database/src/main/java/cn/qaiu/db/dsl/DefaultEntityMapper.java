 ...
 * 默认实体映射器实现
 *
 * <p>基于 Vert.x CodeGen 风格的实体类进行映射 支持各种数据类型的安全转换
 *
 * @param <T> 实体类型
...
public class DefaultEntityMapper<T extends BaseEntity> implements EntityMapper<T> {
  ...
  public DefaultEntityMapper(Class<T> entityClass) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Entity class "
              + entityClass.getName()
              + " must have a constructor that takes JsonObject",
          e);
    }
  ...
  public T fromRecord(Record record) {
    try {
      ...
      LOGGER.error(
          "Failed to map JOQ Record to {}: {}", entityClass.getSimpleName(), e.getMessage(), e);
      return null;
    }
  }
