...
public class ExecutorStrategyRegistry {
  ...
  private final Map<Class<? extends Pool>, ExecutorStrategy> poolStrategies =
  ...
  /** 注册内置执行器策略 包括MySQL、PostgreSQL和H2策略 */
  private void registerBuiltinStrategies() {
    ...
    try {
      ...
      LOGGER.warn(
          "MySQL client not available, skipping MySQL executor strategy: {}", e.getMessage());
    }
    ...
    try {
      ...
      LOGGER.warn(
          "PostgreSQL client not available, skipping PostgreSQL executor strategy: {}",
          e.getMessage());
    }
    ...
    LOGGER.info(
        "Registered builtin executor strategies: H2 (MySQL/PostgreSQL strategies skipped if clients not available)");
  }

  /** 通过SPI加载外部执行器策略 支持动态扩展执行器策略 */
  ...
  public void registerStrategy(ExecutorStrategy strategy) {
    ...
    LOGGER.debug(
        "Registered executor strategy for type: {} and pool: {}", type, poolType.getSimpleName());
  }
  ...
  public ExecutorStrategy getStrategy(Pool pool) {
    ...
    throw new IllegalArgumentException(
        "No executor strategy found for pool: " + pool.getClass().getName());
  }
