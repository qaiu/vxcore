...
/**
 * 支持多数据源的DAO基类 通过注解和上下文实现数据源动态切换
 *
...
public abstract class MultiDataSourceDao {
  ...
  /** 在指定数据源中执行操作 */
  protected <T> Future<T> executeWithDataSource(
      String dataSourceName, java.util.function.Function<JooqExecutor, Future<T>> operation) {
    return Future.future(
        promise -> {
          ...
          try {
            ...
            operation.apply(executor).onSuccess(promise::complete).onFailure(promise::fail);
          } finally {
