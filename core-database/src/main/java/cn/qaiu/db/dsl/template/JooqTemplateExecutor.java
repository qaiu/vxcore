...
/**
 * 基于jOOQ DSL的模板执行器 提供类似vertx-jdbc-template的API，但底层使用jOOQ DSL
 *
 * <p>这是一个简化的实现，展示如何将jOOQ DSL与模板式API结合
 */
public class JooqTemplateExecutor {
  ...
  public <T> Future<List<T>> executeQuery(Query query, EntityMapper<T> mapper) {
    return jooqExecutor.executeQuery(query).map(rowSet -> mapper.fromMultiple(rowSet));
  }

  /** 执行jOOQ Query并映射为单个实体对象 */
  public <T> Future<Optional<T>> executeQueryForSingle(Query query, EntityMapper<T> mapper) {
    return jooqExecutor.executeQuery(query).map(rowSet -> mapper.fromSingle(rowSet));
  }
  ...
  /** 执行SQL模板查询并映射为单个值 */
  public Future<Optional<JsonObject>> queryForOne(
      String sqlTemplate, Map<String, Object> parameters) {
