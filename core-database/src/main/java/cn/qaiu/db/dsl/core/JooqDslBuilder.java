...
/**
 * 简化的 jOOQ DSL SQL 构建器 使用正确的 jOOQ API，避免复杂的类型推断问题
 *
...
public class JooqDslBuilder {
  ...
  public Query buildUpdate(String tableName, JsonObject data, Condition whereCondition) {
    ...
    String sql =
        String.format(
            "UPDATE %s SET %s WHERE %s", tableName, String.join(", ", setClauses), whereClause);
  ...
  public Query buildSelect(String tableName, Condition condition) {
    if (condition != null) {
      ...
      return dslContext.selectFrom(DSL.table(tableName)).where(condition);
    } else {
  ...
  public Query buildCount(String tableName, Condition condition) {
    if (condition != null) {
      // 使用jOOQ的DSL来构建查询，这样可以正确处理参数绑定
      return dslContext.selectCount().from(DSL.table(tableName)).where(condition);
    } else {
      return dslContext.selectCount().from(DSL.table(tableName));
    }
  }
  ...
  public Query buildDelete(String tableName, Condition condition) {
    // 使用jOOQ的DSL来构建查询，这样可以正确处理参数绑定
    return dslContext.deleteFrom(DSL.table(tableName)).where(condition);
  }
   ...
   * @return 分页查询
   */
  public Query buildSelectWithPagination(
      String tableName,
      Condition condition,
      String orderBy,
      boolean ascending,
  ...
  public Query buildBatchInsert(String tableName, List<JsonObject> dataList) {
    ...
    String sql =
        String.format(
            "INSERT INTO %s (%s) VALUES %s",
            tableName, String.join(", ", columns), String.join(", ", valueClauses));
  ...
  public Query buildExists(String tableName, Condition condition) {
    String whereClause = buildConditionString(condition);
    String sql =
        String.format("SELECT EXISTS(SELECT 1 FROM %s WHERE %s) as exists", tableName, whereClause);
  ...
  public Condition buildLikeCondition(String fieldName, String pattern) {
    Field<String> field =
        DSL.field(cn.qaiu.vx.core.util.StringCase.toUnderlineCase(fieldName), String.class);
