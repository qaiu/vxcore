...
/**
 * JService接口的默认实现类 基于EnhancedDao和LambdaDao提供完整的服务层功能
 *
 * @param <T> 实体类型
 * @param <ID> 主键类型
...
public abstract class JServiceImpl<T, ID> extends LambdaDao<T, ID> implements JService<T, ID> {
  ...
  public Future<LambdaDao.BatchUpsertResult<T>> saveOrUpdateBatch(List<T> entities, int batchSize) {
    ...
    for (int i = 0; i < entities.size(); i += batchSize) {
      ...
      List<Future<LambdaDao.UpsertResult<T>>> batchItemFutures =
          batch.stream().map(this::insertOrUpdate).collect(Collectors.toList());

      Future<LambdaDao.BatchUpsertResult<T>> batchFuture =
          Future.all(batchItemFutures)
              .map(
                  compositeFuture -> {
                    int insertCount = 0;
                    int updateCount = 0;
    ...
    return Future.all(batchFutures)
        .map(
            compositeFuture -> {
              ...
              return new LambdaDao.BatchUpsertResult<>(
                  totalInsertCount, totalUpdateCount, allResults);
            });
  }

  @Override
  public Future<LambdaDao.UpsertResult<T>> saveOrUpdateOnConflict(
