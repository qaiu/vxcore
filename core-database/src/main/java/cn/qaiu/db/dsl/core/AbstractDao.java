...
/**
 * 抽象DAO基类 - 基于真正的 jOOQ DSL 充分利用 jOOQ 的类型安全和 DSL 功能
 *
 * <p>支持两种初始化方式： 1. 手动传入JooqExecutor（传统方式） 2. 自动从DataSourceManager获取JooqExecutor（推荐方式）
 */
public abstract class AbstractDao<T, ID> implements JooqDao<T, ID> {
  ...
  public AbstractDao(JooqExecutor executor, Class<T> entityClass) {
    ...
    LOGGER.debug(
        "AbstractDao initialized with manual JooqExecutor for entity: {}",
  ...
  public AbstractDao(Class<T> entityClass) {
    ...
    LOGGER.debug(
        "AbstractDao initialized with auto-executor mode for entity: {} with datasource: {}",
        entityClass.getSimpleName(),
        dataSourceName);
  }
  ...
  public AbstractDao() {
    ...
    LOGGER.debug(
        "AbstractDao initialized with auto-executor mode for entity: {} (from generics) with datasource: {}",
        entityClass.getSimpleName(),
        dataSourceName);
  }
  ...
  protected ID getId(T entity) {
    try {
      ...
      LOGGER.warn(
          "Failed to get ID from entity {}: {}", entityClass.getSimpleName(), e.getMessage(), e);
      return null;
    }
  }
  ...
  private JooqExecutor initializeExecutor() {
    try {
      ...
      LOGGER.info(
          "JooqExecutor auto-initialized for datasource: {} in DAO: {}",
          dataSourceName,
          ...
          entities.stream()
              .map(
                  entity -> {
                    ...
                    // 移除主键字段
                    data.remove(
                        cn.qaiu.db.dsl.core.FieldNameConverter.toDatabaseFieldName(
                            primaryKeyField));
                  ...
                  entity -> {
                    ...
                    if (entityId == null) {
                      throw new IllegalArgumentException(
                          "Entity ID cannot be null for batch update");
                    }
  ...
  /**
   * 批量插入或更新实体（UPSERT） 如果实体ID存在则更新，否则插入
   *
      ...
      return Future.all(insertFuture, updateFuture)
          .map(
              compositeFuture -> {
                ...
                int totalCount =
                    (insertCount != null ? insertCount : 0)
                        + (updateCount != null ? updateCount : 0);
                LOGGER.debug(
                    "Batch upsert completed: {} inserted, {} updated, total: {}",
                    insertCount,
                    updateCount,
                    totalCount);
                return totalCount;
