...
/** 核心jOOQ执行器 基于策略模式实现，支持多种数据库类型 负责将jOOQ Query转换为SQL并通过Vert.x Pool执行 */
public class JooqExecutor {
  ...
  public JooqExecutor(Pool pool) {
    ...
    LOGGER.info(
        "Initialized JooqExecutor with strategy: {} for database type: {}",
  ...
  private void detectDatabaseTypeAsync() {
    pool.getConnection()
            ...
            conn -> {
              try {
                ...
                if (detectedType != null && detectedType != databaseType) {
                  LOGGER.warn(
                      "Detected database type {} differs from strategy type {}, using strategy type",
                      detectedType,
                      databaseType);
                } else {
        ...
        .onFailure(
            throwable -> {
              LOGGER.warn(
                  "Failed to get connection for database type verification: {}",
