...
/**
 * Lambda DAO基类 提供类似MyBatis-Plus的Lambda查询功能
 *
 * @param <T> 实体类型
 * @param <ID> 主键类型
 * @author qaiu
...
public abstract class LambdaDao<T, ID> extends EnhancedDao<T, ID> {
  ...
  public Future<Boolean> lambdaExists(LambdaQueryWrapper<T> wrapper) {
    ...
    return getExecutor().executeQuery(existsSelect).map(rows -> rows.size() > 0);
  }

  /** Lambda查询 - 分页查询 */
  public Future<LambdaPageResult<T>> lambdaPage(
  ...
  public Future<Integer> lambdaDelete(LambdaQueryWrapper<T> wrapper) {
    ...
    DeleteConditionStep<?> delete =
        getExecutor().dsl().deleteFrom(DSL.table(DSL.name(tableName))).where(condition);
  ...
  public Future<Integer> lambdaUpdate(LambdaQueryWrapper<T> wrapper, T entity) {
    ...
    UpdateSetFirstStep<?> update = getExecutor().dsl().update(DSL.table(DSL.name(tableName)));
    ...
    return getExecutor().executeUpdate(whereStep).map(result -> result);
  }
  ...
  public Future<UpsertResult<T>> insertOrUpdate(T entity) {
    ...
    if (primaryKeyValue == null
      ...
      if (primaryKeyFieldLambda == null) {
                            ...
                            insertOptional -> {
                              if (insertOptional.isPresent()) {
                                ...
                                throw new RuntimeException(
                                    "Both update and insert operations failed");
                              }
                            });
                  }
                });
        ...
        SFunction<T, Object> primaryKeyFieldLambdaObj =
            (SFunction<T, Object>) primaryKeyFieldLambda;
  ...
  public Future<BatchUpsertResult<T>> batchInsertOrUpdate(List<T> entities) {
    ...
    List<Future<UpsertResult<T>>> futures = entities.stream().map(this::insertOrUpdate).toList();
  ...
  /**
   * 获取主键字段的Lambda表达式 子类可以重写此方法来提供更精确的主键字段访问
   *
   * <p>注意：此方法返回null，子类需要重写此方法来提供具体的Lambda表达式
   */
  ...
  public static class UpsertResult<T> {
    ...
    public String toString() {
      return "UpsertResult{" + "inserted=" + inserted + ", entity=" + entity + '}';
    }
  }
