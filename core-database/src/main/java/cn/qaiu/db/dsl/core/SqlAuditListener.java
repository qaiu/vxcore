 ...
 * SQL审计监听器
 *
 * <p>提供SQL执行审计功能，包括： - SQL语句记录 - 执行时间统计 - 参数绑定记录 - 性能监控 - 错误追踪
 *
...
public class SqlAuditListener implements ExecuteListener {
  ...
  // 执行统计
  private static final ConcurrentHashMap<String, AtomicLong> queryCounts =
      new ConcurrentHashMap<>();
  private static final ConcurrentHashMap<String, AtomicLong> totalExecutionTime =
      new ConcurrentHashMap<>();
  ...
  public void executeEnd(ExecuteContext ctx) {
    ...
    if (startTime != null) {
      ...
      if (query != null) {
        ...
        // 更新执行时间统计
        totalExecutionTime
            .computeIfAbsent(normalizedSql, k -> new AtomicLong(0))
  ...
  public static class QueryStatistics {
    ...
    public QueryStatistics(
        String sql, long executionCount, long totalExecutionTime, long errorCount) {
      this.sql = sql;
    ...
    public String toString() {
      return String.format(
          "QueryStatistics{sql='%s', count=%d, avgTime=%.2fms, errors=%d, errorRate=%.2f%%}",
