...
public class UserDao extends LambdaDao<User, Long> {
  ...
  public Future<List<User>> findByAgeRange(Integer minAge, Integer maxAge) {
    return lambdaList(lambdaQuery().ge(User::getAge, minAge).le(User::getAge, maxAge));
  }

  /** 根据用户名模糊查询 */
  public Future<List<User>> findByUsernameLike(String username) {
    return lambdaList(lambdaQuery().like(User::getUsername, username));
  }

  /** 查询已认证邮箱的用户 */
  public Future<List<User>> findEmailVerifiedUsers() {
    return lambdaList(lambdaQuery().eq(User::getEmailVerified, true));
  }

  /** 查询余额大于指定金额的用户 */
  public Future<List<User>> findUsersWithBalanceGreaterThan(Double balance) {
    return lambdaList(lambdaQuery().gt(User::getBalance, balance));
  }
  ...
  /** 嵌套条件查询示例 */
  public Future<List<User>> findUsersWithComplexCondition(
      String status, Integer minAge, Double minBalance) {
    return lambdaList(
        lambdaQuery()
  ...
  public Future<LambdaPageResult<User>> findUsersByPage(long current, long size, String status) {
    LambdaQueryWrapper<User> wrapper =
        lambdaQuery().eq(User::getStatus, status).orderByDesc(User::getCreateTime);
  ...
  public Future<Long> countActiveUsers() {
    return lambdaCount(lambdaQuery().eq(User::getStatus, "ACTIVE"));
  }
  ...
  public Future<Integer> updateUserStatus(List<Long> userIds, String newStatus) {
    return lambdaUpdate(lambdaQuery().in(User::getId, userIds), createUserWithStatus(newStatus));
  }

  /** 批量删除示例 - 删除指定状态的用户 */
  public Future<Integer> deleteUsersByStatus(String status) {
    return lambdaDelete(lambdaQuery().eq(User::getStatus, status));
  }
