...
// import java.util.concurrent.ConcurrentHashMap; // 未使用

/**
 * SQL审计统计工具
 *
 * <p>提供SQL执行统计信息的查询和管理功能
 *
...
public class SqlAuditStatistics {
  ...
  public static void printAllStatistics() {
    ...
    LOGGER.info(
        "总体错误率: {:.2f}%", totalExecutions > 0 ? (double) totalErrors / totalExecutions * 100 : 0);
  }
  ...
  public static String exportStatisticsAsJson() {
    ...
    for (SqlAuditListener.QueryStatistics stat : stats.values()) {
      ...
      json.append("      \"totalExecutionTime\": ")
          .append(stat.getTotalExecutionTime())
          .append(",\n");
      json.append("      \"averageExecutionTime\": ")
          .append(stat.getAverageExecutionTime())
          .append(",\n");
