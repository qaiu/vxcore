...
public class ProductDao extends LambdaDao<Product, Long> {
  ...
  public Future<List<Product>> findProductsByComplexCondition(
      Long categoryId, BigDecimal minPrice, Integer minStock) {
    return lambdaList(
        lambdaQuery()
  ...
  /** 分页查询产品 */
  public Future<LambdaPageResult<Product>> findProductsByPage(
      long current, long size, Long categoryId) {
  ...
  public Future<Long> countActiveProducts() {
    return lambdaCount(lambdaQuery().eq(Product::getActive, true));
  }

  /** 统计分类下的产品数量 */
  public Future<Long> countProductsByCategory(Long categoryId) {
    return lambdaCount(
        lambdaQuery().eq(Product::getCategoryId, categoryId).eq(Product::getActive, true));
  }
  ...
  public Future<Integer> updateProductStatus(List<Long> productIds, Boolean active) {
    return lambdaUpdate(
        lambdaQuery().in(Product::getId, productIds), createProductWithStatus(active));
  }

  /** 批量更新产品价格 */
  public Future<Integer> updateProductPrice(List<Long> productIds, BigDecimal newPrice) {
    return lambdaUpdate(
        lambdaQuery().in(Product::getId, productIds), createProductWithPrice(newPrice));
  }

  /** 删除指定分类的产品 */
  public Future<Integer> deleteProductsByCategory(Long categoryId) {
    return lambdaDelete(lambdaQuery().eq(Product::getCategoryId, categoryId));
  }
