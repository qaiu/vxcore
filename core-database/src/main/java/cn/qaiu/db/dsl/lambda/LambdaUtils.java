...
/**
 * Lambda表达式工具类 用于从Lambda表达式中提取字段名和类型信息
 *
 * @author qaiu
 */
public class LambdaUtils {
  ...
  public static <T, R> String getFieldName(SFunction<T, R> column) {
    ...
    return FIELD_NAME_CACHE.computeIfAbsent(
        cacheKey,
        key -> {
          try {
  ...
  public static <T, R> Class<R> getFieldType(SFunction<T, R> column) {
    ...
    Class<R> type =
        (Class<R>)
            FIELD_TYPE_CACHE.computeIfAbsent(
                cacheKey,
                key -> {
                  try {
                    Method method = column.getClass().getDeclaredMethod("writeReplace");
                    ...
                    throw new RuntimeException(
                        "Failed to extract field type from lambda expression", e);
                  }
                });
    return type;
  }
