 ...
 * 查询条件构建器
 *
 * <p>专为复杂查询设计，支持多种查询条件的组合 提供统一的API来构建jOOQ Condition对象
 *
...
public class QueryCondition implements Serializable {
  ...
  public static class ConditionGroup {
    ...
    // Getter methods
    public ConditionType getConnective() {
      return connective;
    }
  ...
  public QueryCondition(JsonObject json) {
    ...
    List<JsonObject> groupJsons =
        json.getJsonArray("groups") != null
            ? json.getJsonArray("groups").getList()
            : new ArrayList<>();
    this.groups = new ArrayList<>();
  ...
  public JsonObject toJson() {
    JsonObject json = new JsonObject().put("field", field).put("type", type).put("value", value);

    if (!groups.isEmpty()) {
      JsonArray groupsJson = new JsonArray();
      for (ConditionGroup group : groups) {
        JsonObject groupJson = new JsonObject().put("connective", group.getConnective());
        // 简化处理，实际需要递归序列化
