...
public class H2ExecutorStrategy extends AbstractExecutorStrategy {
  ...
  public boolean supports(Pool pool) {
    boolean supported = pool instanceof JDBCPool || pool.getClass().getName().contains("JDBCPool");
  ...
  /**
   * 执行插入操作 使用H2特定的IDENTITY()函数获取生成的ID
   *
   * @param pool 连接池
   * @param query 插入查询
   * @return 生成的ID
  ...
  public Future<Long> executeInsert(Pool pool, Query query) {
    ...
    LOGGER.debug(
        "Executing H2 insert: {} with bind values: {}", sql, java.util.Arrays.toString(bindValues));
    ...
    return pool.preparedQuery(sql)
        ...
        .compose(
            result -> {
              ...
              if (rowCount > 0) {
                ...
                System.out.println(
                    "[DEBUG] H2ExecutorStrategy.executeInsert: Insert affected 0 rows");
        ...
        .onFailure(
            error -> {
              System.out.println(
                  "[DEBUG] H2ExecutorStrategy.executeInsert: FAILED - " + error.getMessage());
