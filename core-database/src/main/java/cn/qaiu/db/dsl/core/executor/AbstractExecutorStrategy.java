...
public abstract class AbstractExecutorStrategy implements ExecutorStrategy {
  ...
  public DSLContext createDSLContext(Pool pool) {
    ...
    LOGGER.debug("Created DSL context for {} with dialect {}", getSupportedType(), getSQLDialect());
  ...
  public Future<RowSet<io.vertx.sqlclient.Row>> executeQuery(Pool pool, Query query) {
    ...
    LOGGER.debug(
        "Executing query: {} with bind values: {}", sql, java.util.Arrays.toString(bindValues));

    return pool.preparedQuery(sql)
        .execute(io.vertx.sqlclient.Tuple.from(bindValues))
        .onSuccess(
            result -> LOGGER.debug("Query executed successfully, affected rows: {}", result.size()))
  ...
  public Future<Integer> executeUpdate(Pool pool, Query query) {
    ...
    LOGGER.debug(
        "Executing update: {} with bind values: {}", sql, java.util.Arrays.toString(bindValues));
  ...
  /**
   * 执行插入操作 使用通用的IDENTITY()函数获取生成的ID
   *
   * @param pool 连接池
   * @param query 插入查询对象
   * @return 生成的ID
  ...
  public Future<Long> executeInsert(Pool pool, Query query) {
    ...
    LOGGER.debug(
        "Executing insert: {} with bind values: {}", sql, java.util.Arrays.toString(bindValues));
    ...
    return pool.preparedQuery(sql)
        ...
        .compose(
            result -> {
              ...
              System.out.println(
                  "[DEBUG] AbstractExecutorStrategy.executeInsert: rowCount=" + rowCount);
              ...
              if (rowCount > 0) {
                ...
                System.out.println(
                    "[DEBUG] AbstractExecutorStrategy.executeInsert: calling IDENTITY()");
                return pool.query("SELECT IDENTITY()")
                    .execute()
                    .map(
                        idResult -> {
                          System.out.println(
                              "[DEBUG] AbstractExecutorStrategy.executeInsert: IDENTITY() size="
                                  + idResult.size());
                          if (idResult.size() > 0) {
                            io.vertx.sqlclient.Row row = idResult.iterator().next();
                            Object id = row.getValue(0);
                            System.out.println(
                                "[DEBUG] AbstractExecutorStrategy.executeInsert: ID="
                                    + id
                                    + ", type="
                            ...
                            if (id instanceof Number) {
                              ...
                              System.out.println(
                                  "[DEBUG] AbstractExecutorStrategy.executeInsert: generatedId="
                                      + generatedId);
                          ...
                          System.out.println(
                              "[DEBUG] AbstractExecutorStrategy.executeInsert: returning 0");
                          LOGGER.warn("Could not retrieve generated ID");
                          return 0L;
                        });
              } else {
                System.out.println(
                    "[DEBUG] AbstractExecutorStrategy.executeInsert: rowCount=0, returning 0");
        ...
        .onFailure(
            error -> {
              System.out.println(
                  "[DEBUG] AbstractExecutorStrategy.executeInsert: FAILED - " + error.getMessage());
              ...
              try {
                ...
                java.util.List<
                        io.vertx.sqlclient.PreparedQuery<
                            io.vertx.sqlclient.RowSet<io.vertx.sqlclient.Row>>>
                    preparedQueries = new java.util.ArrayList<>();
                ...
                return batchFuture
                    .onSuccess(
                        result -> {
                          LOGGER.debug(
                              "Batch operation completed successfully, total affected rows: {}",
