...
public class JDBCPoolInit {
  ...
  private static final String providerClass =
      io.vertx.ext.jdbc.spi.impl.HikariCPDataSourceProvider.class.getName();
   ...
   * 这个方法只允许调用一次
   */
  public synchronized Future<Void> initPool() {
    if (pool != null) {
    ...
    return EnhancedCreateTable.createTableWithStrictMapping(pool, type);
  }
   ...
   * @return pool
   */
  public synchronized JDBCPool getPool() {
    return pool;
  }
}
