...
public class TableStructureComparator {
  ...
  /** 比较表结构并返回差异列表 */
  public static Future<List<TableDifference>> compareTableStructure(
      Pool pool, TableMetadata expectedMetadata, JDBCType dbType) {
  ...
  private static void compareColumns(
      ...
      JDBCType dbType,
      List<TableDifference> differences,
      Promise<List<TableDifference>> promise) {
    ...
    pool.query(columnsSql)
                  ...
                  differences.add(
                      new TableDifference(
                          ...
                          generateAddColumnSql(
                              expectedMetadata.getTableName(), expectedColumn, dbType)));
                } else {
                  // 比较列属性
                  ...
                  differences.add(
                      new TableDifference(
                          ...
                          generateDropColumnSql(
                              expectedMetadata.getTableName(), actualColumn.getName(), dbType)));
                }
              }
        ...
        .onFailure(
            throwable -> {
              LOGGER.error(
                  "Failed to compare columns for table: {}",
                  expectedMetadata.getTableName(),
                  throwable);
  ...
  /** 比较列属性 */
  private static void compareColumnProperties(
      String tableName,
  ...
  private static boolean isTypeEquivalent(String expected, String actual) {
    ...
    return switch (expected) {
      ...
      case "text" -> actual.equals("character varying")
          || actual.equals("text")
          || actual.equals("clob");
      case "timestamp" -> actual.equals("datetime")
          || actual.equals("timestamp")
          || actual.equals("timestamp without time zone")
          || actual.equals("timestamp with time zone")
          || actual.equals("timestamptz")
          || actual.equals("timestamp(6)");
      case "datetime" -> actual.equals("timestamp")
          || actual.equals("datetime")
          || actual.equals("timestamp without time zone")
          || actual.equals("timestamp with time zone")
          || actual.equals("timestamptz")
          || actual.equals("datetime(6)");
      case "timestamp without time zone" -> actual.equals("timestamp")
          || actual.equals("datetime")
          || actual.equals("timestamp without time zone")
          || actual.equals("timestamp(6)");
      case "timestamp with time zone" -> actual.equals("timestamptz")
          || actual.equals("timestamp with time zone");
      case "time" -> actual.equals("time")
          || actual.equals("time without time zone")
          || actual.equals("time with time zone")
          || actual.equals("timetz");
      case "time without time zone" -> actual.equals("time")
          || actual.equals("time without time zone");
  ...
  /** 检查可空性是否兼容 */
  private static boolean isNullableCompatible(
      ColumnMetadata expected, ColumnInfo actual, JDBCType dbType) {
    // 对于主键字段，采用更宽松的策略
  ...
  private static String generateCreateTableSql(TableMetadata metadata, JDBCType dbType) {
    ...
    if (dbType == JDBCType.MySQL) {
      sb.append(" ENGINE=")
          .append(metadata.getEngine())
          .append(" DEFAULT CHARSET=")
          .append(metadata.getCharset())
          .append(" COLLATE=")
          .append(metadata.getCollate());
    } else if (dbType == JDBCType.PostgreSQL) {
  ...
  /** 生成修改列类型的SQL */
  private static String generateAlterColumnTypeSql(
      String tableName, ColumnMetadata column, JDBCType dbType) {
    String quotationMarks = dbType == JDBCType.MySQL ? "`" : "\"";
    ...
    if (dbType == JDBCType.PostgreSQL) {
      ...
      sql.append(
          String.format(
              "ALTER TABLE %s%s%s ALTER COLUMN %s%s%s TYPE %s",
              quotationMarks,
              tableName,
              quotationMarks,
              quotationMarks,
      ...
      return String.format(
          "ALTER TABLE %s%s%s MODIFY COLUMN %s",
          quotationMarks, tableName, quotationMarks, column.toColumnDefinition(dbType, false));
    }
  }

  /** 生成修改列长度的SQL */
  private static String generateAlterColumnLengthSql(
      String tableName, ColumnMetadata column, JDBCType dbType) {
    return generateAlterColumnTypeSql(tableName, column, dbType);
  }

  /** 生成修改列可空性的SQL */
  private static String generateAlterColumnNullableSql(
      String tableName, ColumnMetadata column, JDBCType dbType) {
    return generateAlterColumnTypeSql(tableName, column, dbType);
  }

  /** 生成修改列默认值的SQL */
  private static String generateAlterColumnDefaultSql(
      String tableName, ColumnMetadata column, JDBCType dbType) {
    String quotationMarks = dbType == JDBCType.MySQL ? "`" : "\"";
    ...
    if (dbType == JDBCType.PostgreSQL) {
      ...
      if (column.getDefaultValue() != null && !column.getDefaultValue().isEmpty()) {
        ...
        if (column.isDefaultValueIsFunction()) {
          ...
          sql.append(
              String.format(
                  "ALTER TABLE %s%s%s ALTER COLUMN %s%s%s SET DEFAULT %s",
                  quotationMarks,
                  tableName,
                  quotationMarks,
                  quotationMarks,
        ...
        sql.append(
            String.format(
                "ALTER TABLE %s%s%s ALTER COLUMN %s%s%s DROP DEFAULT",
                quotationMarks,
                tableName,
                quotationMarks,
                quotationMarks,
  ...
  private static class ColumnInfo {
    ...
    public String toString() {
      return String.format(
          "ColumnInfo{name='%s', type='%s', length=%d, nullable=%s, defaultValue='%s', autoIncrement=%s}",
