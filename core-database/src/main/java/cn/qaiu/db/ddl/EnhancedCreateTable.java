...
public class EnhancedCreateTable {
  ...
  public static Future<List<TableStructureComparator.TableDifference>> synchronizeTables(
      Pool pool) {
  ...
  /** 同步指定的表 */
  public static Future<List<TableStructureComparator.TableDifference>> synchronizeTable(
      Pool pool, Class<?> clz, JDBCType type) {
  ...
  public static Future<Boolean> hasTablesNeedingSync(Pool pool, JDBCType type) {
    ...
    List<Future<Boolean>> futures =
        ddlTableClasses.stream().map(clz -> needsSynchronization(pool, clz, type)).toList();
  ...
  public static Future<String> generateTableStructureReport(Pool pool, JDBCType type) {
    ...
    for (Class<?> clz : ddlTableClasses) {
      Future<Void> future =
          needsSynchronization(pool, clz, type)
              .onSuccess(
                  needsSync -> {
                    report
                        .append("Table: ")
