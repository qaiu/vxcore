...
public class CreateDatabase {
  ...
  public static void createDatabase(String url, String user, String password) {
    ...
    // 去掉数据库名，构建不带数据库名的URL
    String baseUrl =
        url.substring(0, url.lastIndexOf("/") + 1) + "?characterEncoding=UTF-8&useUnicode=true";
  ...
  public static void createDatabase(JsonObject dbConfig) {
    createDatabase(
        dbConfig.getString("jdbcUrl"), dbConfig.getString("user"), dbConfig.getString("password"));
  }
}
