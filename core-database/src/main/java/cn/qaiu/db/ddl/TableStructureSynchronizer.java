...
public class TableStructureSynchronizer {
  ...
  public static Future<List<TableStructureComparator.TableDifference>> synchronizeAllTables(
    ...
    try {
      ...
      List<Future<List<TableStructureComparator.TableDifference>>> futures =
          ddlTableClasses.stream().map(clz -> synchronizeTable(pool, clz, dbType)).toList();
              ...
              result -> {
                ...
                for (Object futureResult : result.list()) {
                  if (futureResult instanceof List<?> list) {
                    ...
                    List<TableStructureComparator.TableDifference> differences =
                        (List<TableStructureComparator.TableDifference>) list;
  ...
  /** 同步单个表结构 */
  public static Future<List<TableStructureComparator.TableDifference>> synchronizeTable(
      Pool pool, Class<?> clz, JDBCType dbType) {
    Promise<List<TableStructureComparator.TableDifference>> promise = Promise.promise();
    ...
    try {
      ...
      LOGGER.info(
          "Synchronizing table: {} (version: {})", metadata.getTableName(), metadata.getVersion());
              ...
              differences -> {
                ...
                executeSynchronization(pool, differences, metadata.getTableName())
                    .onSuccess(
                        v -> {
                          LOGGER.info(
                              "Successfully synchronized table: {}", metadata.getTableName());
                          promise.complete(differences);
                        })
                    .onFailure(promise::fail);
              })
  ...
  /** 执行同步操作 */
  private static Future<Void> executeSynchronization(
      Pool pool, List<TableStructureComparator.TableDifference> differences, String tableName) {
  ...
  /** 串行执行差异修复 */
  private static void executeDifferencesSequentially(
      Pool pool,
      List<TableStructureComparator.TableDifference> differences,
      int index,
    ...
    if (index >= differences.size()) {
      LOGGER.info(
          "Successfully executed {} synchronization operations for table {}",
          differences.size(),
          differences.isEmpty() ? "unknown" : differences.get(0).getTableName());
      promise.complete();
      return;
    }
    ...
    executeDifference(pool, diff)
        ...
        .onFailure(
            throwable -> {
              LOGGER.error(
                  "Failed to execute difference {} for table {}: {}",
                  index,
                  diff.getTableName(),
  ...
  /** 执行单个差异修复 */
  private static Future<Void> executeDifference(
      Pool pool, TableStructureComparator.TableDifference difference) {
    Promise<Void> promise = Promise.promise();
    ...
    if (sql == null) {
      LOGGER.debug(
          "Skipping SQL execution for {}: {} (no SQL needed)",
          difference.getType(),
    ...
    if (sql.contains(";\n")) {
      ...
      Future.all(futures)
          .onSuccess(
              result -> {
                LOGGER.debug(
                    "Successfully executed all SQL statements for {}: {}",
                    difference.getType(),
                    difference.getColumnName());
                promise.complete();
              })
          .onFailure(
              throwable -> {
                LOGGER.error(
                    "Failed to execute SQL statements for {}: {}",
                    difference.getType(),
                    throwable.getMessage());
                promise.fail(throwable);
              });
  ...
  public static Future<Boolean> needsSynchronization(Pool pool, Class<?> clz, JDBCType dbType) {
    ...
    try {
      ...
      if (metadata.getDbType() != dbType) {
        LOGGER.debug(
            "Table {} database type mismatch: expected {}, got {}",
            metadata.getTableName(),
            dbType,
            metadata.getDbType());
      ...
      TableStructureComparator.compareTableStructure(pool, metadata, dbType)
          .onSuccess(
              differences -> {
                ...
                LOGGER.debug(
                    "Table {} needs synchronization: {}", metadata.getTableName(), needsSync);
