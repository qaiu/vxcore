...
public class CreateTable {
  ...
  public static List<String> getCreateTableSQL(Class<?> clz, JDBCType type) {
    ...
    sb.append("CREATE TABLE IF NOT EXISTS ")
        .append(tableInfo.quotationMarks)
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to generate CREATE TABLE SQL for class: "
              + clz.getName()
              + ", error String: '"
              + sb
              + "'",
  ...
  private static void processFields(
      Class<?> clz, TableInfo tableInfo, StringBuilder sb, List<String> indexSQLs) {
    ...
    for (Field field : fields) {
      ...
      // 构建列定义
      sb.append(tableInfo.quotationMarks)
          .append(column)
          .append(tableInfo.quotationMarks)
          .append(" ")
          .append(sqlType);
  ...
  // 添加类型长度
  private static void appendTypeLength(
      String sqlType, StringBuilder sb, int[] decimalSize, int varcharSize) {
    ...
    if (field.isAnnotationPresent(Constraint.class)) {
      ...
      if (constraint.autoIncrement() && type == JDBCType.PostgreSQL) {
        ...
        if (StringUtils.isNotEmpty(constraint.defaultValue())) {
          sb.append(" DEFAULT ")
              .append(apostrophe)
              .append(constraint.defaultValue())
              .append(apostrophe);
        }
  ...
  private static void appendIndex(TableInfo tableInfo, List<String> indexSQLs, Field field) {
    ...
    // 检查是否已有相同索引名称的索引
    Optional<String> existingIndex =
        indexSQLs.stream()
            .filter(
                sql ->
                    sql.contains(tableInfo.quotationMarks + indexName + tableInfo.quotationMarks))
            .findFirst();
