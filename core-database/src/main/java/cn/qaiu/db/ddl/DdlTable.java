...
/**
 * 严格的DDL表映射注解 用于实现Java对象与数据库表的严格映射和自动同步
 *
...
public @interface DdlTable {
  ...
  /**
   * 数据库类型 用于自动识别数据库类型，支持：mysql, postgresql, h2, oracle, sqlserver
   *
   * @deprecated 此字段已过时，现在优先使用JDBC Pool的数据库类型自动检测。 系统会从Pool连接中自动获取数据库类型，无需在注解中指定。 优先级：Pool的JDBC类型 >
   *     注解中的类型
