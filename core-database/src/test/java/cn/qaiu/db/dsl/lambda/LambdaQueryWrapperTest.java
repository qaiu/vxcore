...
class LambdaQueryWrapperTest {
  ...
  class BasicConditionTest {
    ...
    void testNotBetween() {
      ...
      String sql = wrapper.buildSelect().getSQL();
      assertTrue(
          sql.contains("balance not between cast(? as double) and cast(? as double)"),
          "SQL应包含not between条件");
    }
  }
  ...
  class NestedConditionTest {
    ...
    void testAndNested() {
      wrapper
          ...
          .and(subWrapper -> subWrapper.ge(User::getAge, 18).le(User::getAge, 65));
    ...
    void testOrNested() {
      wrapper
          ...
          .or(
              subWrapper ->
                  subWrapper.eq(User::getStatus, "PENDING").eq(User::getEmailVerified, true));
  ...
  class OrderByTest {
    ...
    void testMultipleOrderBy() {
      wrapper.orderByDesc(User::getBalance).orderByAsc(User::getCreateTime);
      String sql = wrapper.buildSelect().getSQL();
  ...
  class PaginationTest {
    ...
    void testPageCalculation() {
      LambdaQueryWrapper<User> pageWrapper =
  ...
  class QueryBuildTest {
    ...
    void testBuildCondition() {
      wrapper.eq(User::getStatus, "ACTIVE").gt(User::getAge, 18);
    ...
    void testBuildSelect() {
      wrapper.eq(User::getStatus, "ACTIVE").orderByDesc(User::getCreateTime).limit(10);
  ...
  class RealWorldScenarioTest {
          ...
          .and(
              subWrapper ->
                  subWrapper
                      ...
                      .or(
                          subSubWrapper ->
                              subSubWrapper.ge(User::getBalance, 500.0).eq(User::getAge, 25)));
    ...
    void testPaginationScenario() {
      // 模拟分页查询：第3页，每页15条记录
      wrapper.eq(User::getStatus, "ACTIVE").orderByDesc(User::getCreateTime).page(3, 15);
    ...
    void testComplexQueryScenario() {
      ...
      wrapper
          .select(User::getId, User::getUsername, User::getEmail, User::getBalance)
          .eq(User::getStatus, "ACTIVE")
          .ge(User::getAge, 21)
          .le(User::getAge, 60)
