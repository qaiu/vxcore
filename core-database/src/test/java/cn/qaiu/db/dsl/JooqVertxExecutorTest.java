...
public class JooqVertxExecutorTest {
  ...
  void testSimpleSelect(VertxTestContext testContext) {
    // 插入测试数据
    pool.query("INSERT INTO test_table (name, age) VALUES ('Test', 25)")
        .execute()
        .compose(
            v -> {
  ...
  void testInsert(VertxTestContext testContext) {
    pool.query(
            "INSERT INTO test_table (name, age, email) VALUES ('Insert Test', 30, 'insert@test.com')")
        .execute()
        .onSuccess(
            result -> {
              testContext.verify(
  ...
  void testUpdate(VertxTestContext testContext) {
    // 先插入测试数据
    pool.query("INSERT INTO test_table (name, age) VALUES ('Update Test', 20)")
        .execute()
        .compose(
            insertResult -> {
              // 然后更新数据
  ...
  void testComplexSelect(VertxTestContext testContext) {
    // 插入多条测试数据
    pool.query(
            "INSERT INTO test_table (name, age, email) VALUES ('User1', 20, 'user1@example.com')")
        .execute()
        .compose(
            v ->
                pool.query(
                        "INSERT INTO test_table (name, age, email) VALUES ('User2', 25, 'user2@example.com')")
                    .execute())
        .compose(
            v ->
                pool.query(
                        "INSERT INTO test_table (name, age, email) VALUES ('User3', 30, 'user3@example.com')")
                    .execute())
        .compose(
            v -> {
              // 查询年龄范围
