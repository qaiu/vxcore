...
public class DdlColumnValueTest {
  ...
  private void insertTestData() {
    ...
    pool.query(insertSql)
        .execute()
        .onSuccess(
            result -> logger.debug("Products test data inserted: {} rows", result.rowCount()))
  ...
  void testBatchUpdateQueries() {
    ...
    productDao
        .updateProductStatus(productIds, false)
        .onSuccess(
            updatedCount -> {
              ...
              // 验证更新结果
              productDao
                  .lambdaList(Product::getActive, false)
                  .onSuccess(
                      updatedProducts -> {
    ...
    productDao
        ...
        .onSuccess(
            inserted -> {
              // 测试IS NULL查询
              productDao
                  .lambdaList(productDao.lambdaQuery().isNull(Product::getDescription))
                  .onSuccess(
                      products -> {
                        assertTrue(products.size() >= 1);
                        products.forEach(
