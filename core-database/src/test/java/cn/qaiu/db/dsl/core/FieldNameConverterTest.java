...
class FieldNameConverterTest {
  ...
  class ToDatabaseFieldNameTest {
    ...
    void testParameterizedFieldNameConversion(
        String javaFieldName, String expectedDatabaseFieldName) {
      assertEquals(
          expectedDatabaseFieldName, FieldNameConverter.toDatabaseFieldName(javaFieldName));
    }
  }
  ...
  class ToJavaFieldNameTest {
    ...
    @DisplayName("参数化字段名转换测试")
    void testParameterizedFieldNameConversion(
        String databaseFieldName, String expectedJavaFieldName) {
  ...
  class RoundTripTest {
    ...
    void testJavaToDatabaseRoundTrip() {
      ...
      for (String javaFieldName : javaFieldNames) {
        ...
        assertEquals(javaFieldName, backToJava, "往返转换应该保持一致: " + javaFieldName);
      }
    }
    ...
    void testDatabaseToJavaRoundTrip() {
      ...
      for (String databaseFieldName : databaseFieldNames) {
        ...
        assertEquals(databaseFieldName, backToDatabase, "往返转换应该保持一致: " + databaseFieldName);
      }
    }
  }
  ...
  class RealWorldScenarioTest {
    ...
    void testDatabaseTableNameConversion() {
      ...
      for (String className : entityClassNames) {
        ...
        assertTrue(tableName.matches("^[a-z][a-z0-9_]*$"), "表名应符合数据库命名规范: " + tableName);
      }
    }
  }
