...
public class LambdaQueryTest {
  ...
  void testBasicLambdaQuery() {
    ...
    // 测试等值查询
    userDao
        .lambdaOne(User::getUsername, "john_doe")
        .onSuccess(
            user -> {
  ...
  void testLikeQuery() {
    ...
    // 测试模糊查询
    userDao
        .lambdaList(userDao.lambdaQuery().like(User::getUsername, "john"))
        .onSuccess(
            users -> {
  ...
  void testComplexQuery() {
    ...
    // 测试多条件组合查询
    userDao
        .findActiveUsersWithHighBalance(1000.0)
        .onSuccess(
            users -> {
              assertTrue(users.size() >= 2);
              users.forEach(
  ...
  void testNestedConditionQuery() {
    ...
    // 测试嵌套条件查询
    userDao
        .findUsersWithComplexCondition("ACTIVE", 25, 1000.0)
        .onSuccess(
            users -> {
                    ...
                    assertTrue(
                        user.getAge() >= 25
                            || (user.getBalance() != null
                                && user.getBalance().doubleValue() >= 1000.0
  ...
  void testBatchUpdate() {
    ...
    userDao
        .updateUserStatus(userIds, "UPDATED")
        .onSuccess(
            updatedCount -> {
              ...
              // 验证更新结果
              userDao
                  .lambdaList(User::getStatus, "UPDATED")
                  .onSuccess(
                      updatedUsers -> {
  ...
  void testInQuery() {
    ...
    userDao
        .lambdaList(userDao.lambdaQuery().in(User::getStatus, statuses))
        .onSuccess(
            users -> {
              assertTrue(users.size() >= 3);
              users.forEach(
    ...
    userDao
        ...
        .onSuccess(
            inserted -> {
              ...
              // 测试IS NULL查询
              userDao
                  .lambdaList(userDao.lambdaQuery().isNull(User::getBio))
                  .onSuccess(
                      users -> {
                        assertTrue(users.size() >= 1);
                        users.forEach(
