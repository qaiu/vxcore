...
public class UserDao extends LambdaDao<User, Long> {
  ...
  public Future<List<User>> findByAgeRange(int minAge, int maxAge) {
    return lambdaQuery().ge(User::getAge, minAge).le(User::getAge, maxAge).list();
  }
  ...
  public Future<List<User>> findUsersWithComplexCondition(
      String status, int minAge, double minBalance) {
    return lambdaQuery()
  ...
  public Future<LambdaPageResult<User>> findUsersByPage(int page, int size, String status) {
    return lambdaPage(
        lambdaQuery().eq(User::getStatus, status).orderByDesc(User::getCreateTime), page, size);
  }
