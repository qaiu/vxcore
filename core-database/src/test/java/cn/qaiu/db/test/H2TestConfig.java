...
/**
 * H2数据库测试配置 确保测试数据隔离和最大兼容性
 *
 * @author QAIU
 */
public class H2TestConfig {

  /** 创建H2内存数据库连接池 每个测试使用独立的数据库实例，避免数据污染 */
  ...
  public static JDBCPool createH2Pool(Vertx vertx, String dbName) {
    PoolOptions poolOptions = new PoolOptions().setMaxSize(5).setMaxWaitQueueSize(10);
    ...
    JDBCConnectOptions connectOptions =
        new JDBCConnectOptions().setJdbcUrl(jdbcUrl).setUser("sa").setPassword("");
      ...
      for (String table : tables) {
        pool.query("DROP TABLE IF EXISTS " + table)
            ...
            .onFailure(
                error -> {
                  // 忽略表不存在的错误
                  if (!error.getMessage().contains("Table")
