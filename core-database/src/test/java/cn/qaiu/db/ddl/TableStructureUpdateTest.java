...
public class TableStructureUpdateTest {
      ...
      pool.query(createSQL)
          ...
          .onSuccess(
              result -> {
                ...
                // 验证表是否创建成功
                pool.query(
                        "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'EXAMPLE_USER'")
                    .execute()
                    .onSuccess(
                        countResult -> {
                          ...
                          if (tableCount > 0) {
                            // 验证初始字段数量 - 使用H2兼容的查询
                            pool.query(
                                    "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'EXAMPLE_USER'")
                                .execute()
                                .onSuccess(
                                    columnCountResult -> {
                                      int columnCount =
                                          columnCountResult.iterator().next().getInteger(0);
                                      System.out.println("表EXAMPLE_USER的字段数量: " + columnCount);

                                      System.out.println(
                                          "✅ 第一步完成：初始表结构创建成功，包含" + columnCount + "个字段");
                                      testContext.completeNow();
                                    })
                                .onFailure(
                                    e -> {
                                              ...
                                              columnCountResult2 -> {
                                                int columnCount =
                                                    columnCountResult2
                                                        ...
                                                        .getInteger(0);
                                                System.out.println(
                                                    "表example_user的字段数量: " + columnCount);
                            ...
                            pool.query(
                                ...
                                .onSuccess(
                                    countResult2 -> {
                                      int tableCount2 =
                                          countResult2.iterator().next().getInteger(0);
        ...
        .onSuccess(
            columnCountResult -> {
              ...
              if (columnCount >= 10) { // 至少应该有基础字段
                // 验证新增的字段是否存在
                pool.query(
                        "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'extended_user' ORDER BY COLUMN_NAME")
                    .execute()
                    .onSuccess(
                        columnsResult -> {
                          // 检查是否包含新增的字段
  ...
  private void addMissingFields(
      VertxTestContext testContext, boolean hasPhone, boolean hasAddress, boolean hasBirthday) {
      ...
      pool.query(createBaseSQL)
          ...
          .compose(
              result -> {
                ...
                return TableStructureSynchronizer.synchronizeTable(
    ...
    pool.query("SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'extended_user'")
        ...
        .onSuccess(
            tableResult -> {
              ...
              // 检查字段数量和具体字段
              pool.query(
                      "SELECT COLUMN_NAME, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'extended_user' ORDER BY COLUMN_NAME")
                  .execute()
                  .onSuccess(
                      columnsResult -> {
  ...
  /** 扩展的用户实体类 - 用于测试表结构更新 在ExampleUser基础上新增3个字段 */
  @DdlTable(
