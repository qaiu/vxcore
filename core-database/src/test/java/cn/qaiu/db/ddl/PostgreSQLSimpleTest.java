  ...
  public void testPostgreSQLCreateTableSql(VertxTestContext testContext) {
    ...
    testContext.verify(
        () -> {
          ...
          assertTrue(
              createTableSql.contains("\"example_user\""), "Should contain table name with quotes");

          // 验证SERIAL类型
          assertTrue(
              createTableSql.contains("SERIAL") || createTableSql.contains("BIGSERIAL"),
              "Should contain SERIAL or BIGSERIAL for auto-increment columns");

          // 验证没有NOT SERIAL这样的错误语法
          assertTrue(
              !createTableSql.contains("NOT SERIAL"),
