...
public class MultiDataSourceTest {
  ...
  void testDataSourceManagerRegistration() throws InterruptedException {
    ...
    DataSourceConfig config = new DataSourceConfig("test", "h2", "jdbc:h2:mem:testdb", "sa", "");

    dataSourceManager
        .registerDataSource("test", config)
        .onSuccess(
            v -> {
  ...
  void testDataSourceManagerInitialization() throws InterruptedException {
    ...
    DataSourceConfig config =
        new DataSourceConfig("test", "h2", "jdbc:h2:mem:testdb;DB_CLOSE_DELAY=-1", "sa", "");
  ...
  void testH2DataSourceProvider() throws InterruptedException {
    ...
    DataSourceConfig config =
        new DataSourceConfig(
            "test",
            "h2",
            "jdbc:h2:mem:testdb;DB_CLOSE_DELAY=-1;MODE=MySQL;DATABASE_TO_LOWER=TRUE;CASE_INSENSITIVE_IDENTIFIERS=TRUE;NON_KEYWORDS=VALUE",
            "sa",
            "");
    ...
    provider
              ...
              pool.getConnection()
                  ...
                  .onFailure(
                      error -> {
                        ...
                        System.out.println(
                            "Connection test failed (expected in test environment): "
                                + error.getMessage());
                        latch.countDown();
                      });
            })
        .onFailure(
            error -> {
              ...
              System.out.println(
                  "Pool creation failed (expected in test environment): " + error.getMessage());
              latch.countDown();
            });
