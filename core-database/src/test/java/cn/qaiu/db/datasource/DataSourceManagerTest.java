...
class DataSourceManagerTest {
  ...
  class DataSourceRegistrationTest {
    ...
    void testRegisterDataSourceFromJson(VertxTestContext testContext) {
      ...
      dataSourceManager
          .registerDataSource(name, configJson)
          .onSuccess(
              v -> {
                assertNotNull(dataSourceManager.getDataSourceConfig(name), "数据源配置应该已注册");
                testContext.completeNow();
    ...
    void testRegisterDuplicateDataSource(VertxTestContext testContext) {
      ...
      dataSourceManager
          .registerDataSource(name, config1)
    ...
    void testInitializeNonExistentDataSource(VertxTestContext testContext) {
      ...
      dataSourceManager
          ...
          .onFailure(
              error -> {
                assertTrue(
                    error.getMessage().contains("DataSource config not found"), "应该返回配置未找到的错误");
                testContext.completeNow();
              });
    }
  ...
  class DataSourceManagementTest {
    ...
    void testCloseAllDataSources(VertxTestContext testContext) {
      ...
      dataSourceManager
          .registerDataSource(name1, config1)
  ...
  class ErrorHandlingTest {
              ...
              error -> {
                testContext.verify(
                    () -> {
                      ...
                      assertTrue(
                          error.getMessage().contains("No provider found")
    ...
    void testUnsupportedDatabaseType(VertxTestContext testContext) {
      ...
      dataSourceManager
          ...
          .onFailure(
              error -> {
                assertTrue(error.getMessage().contains("No provider found"), "应该返回提供者未找到的错误");
                testContext.completeNow();
              });
    }
  }
      ...
      Future.all(futures)
          .onSuccess(
              v -> {
                for (int i = 0; i < count; i++) {
                  String name = "concurrent-db-" + i;
                  assertNotNull(dataSourceManager.getDataSourceConfig(name), "数据源配置应该存在: " + name);
                }
                testContext.completeNow();
              })
          .onFailure(testContext::failNow);
