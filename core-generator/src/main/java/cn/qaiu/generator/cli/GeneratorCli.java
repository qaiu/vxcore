...
public class GeneratorCli implements Callable<Integer> {
  ...
  @Option(
      names = {"--dao-style"},
      description = "DAO风格: vertx, jooq, lambda")
  ...
  @Option(
      names = {"--no-service"},
      description = "不生成Service")
  private boolean noService = false;

  @Option(
      names = {"--no-controller"},
      description = "不生成Controller")
  ...
  public Integer call() throws Exception {
    try {
      ...
      List<String> generatedFiles =
          generator.generateAll(tables).toCompletionStage().toCompletableFuture().get();
