...
/**
 * JDBC 数据库元数据读取器实现 支持 MySQL、PostgreSQL、H2 数据库
 *
 * @author QAIU
 */
public class JdbcMetadataReader implements DatabaseMetadataReader {
                      ...
                      try (ResultSet rs =
                        ...
                        while (rs.next()) {
                          ...
                          List<ForeignKeyInfo> foreignKeys =
                              readForeignKeysSync(conn, schema, tableName);
              ...
              return Future.future(
                  promise -> {
                    try {
                      ...
                      try (ResultSet rs =
                          metaData.getTables(null, schema, tableName, new String[] {"TABLE"})) {
                        if (rs.next()) {
  ...
  /** 同步读取列信息 */
  private List<ColumnInfo> readColumnsSync(Connection conn, String schema, String tableName)
      throws SQLException {
    DatabaseMetaData metaData = conn.getMetaData();
  ...
  /** 同步读取索引信息 */
  private List<IndexInfo> readIndexesSync(Connection conn, String schema, String tableName)
      throws SQLException {
    DatabaseMetaData metaData = conn.getMetaData();
  ...
  /** 同步读取外键信息 */
  private List<ForeignKeyInfo> readForeignKeysSync(Connection conn, String schema, String tableName)
      throws SQLException {
    DatabaseMetaData metaData = conn.getMetaData();
