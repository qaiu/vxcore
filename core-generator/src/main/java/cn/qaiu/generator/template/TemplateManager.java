...
public class TemplateManager {
    ...
    return Future.future(
        promise -> {
          try {
            ...
            // 生成代码
            codeGenerator
                .generate(templateName, context, outputPath)
                .onSuccess(
                    result -> {
                      LOGGER.info(
                          "Code generated successfully: {} -> {}", templateName, outputPath);
                      promise.complete(result);
                    })
                .onFailure(
                    error -> {
                      LOGGER.error(
                          "Failed to generate code: template={}, output={}",
                          templateName,
                          outputPath,
                          error);
                      promise.fail(error);
            ...
            LOGGER.error(
                "Failed to generate code: template={}, output={}", templateName, outputPath, e);
            promise.fail(e);
          }
        });
  }
    ...
    return Future.future(
        promise -> {
          try {
            ...
            // 生成代码
            codeGenerator
                .generateToString(templateName, context)
                .onSuccess(
                    result -> {
                ...
                .onFailure(
                    error -> {
                      LOGGER.error(
                          "Failed to generate code to string: template={}", templateName, error);
                      promise.fail(error);
                    });
  ...
  private TemplateContext buildTemplateContext(EntityInfo entityInfo) {
    ...
    packageInfo.put(
        "basePackage",
        entityInfo.getPackageName().substring(0, entityInfo.getPackageName().lastIndexOf('.')));
    ...
    packageInfo.put(
        "controllerPackage", entityInfo.getPackageName().replace("entity", "controller"));
