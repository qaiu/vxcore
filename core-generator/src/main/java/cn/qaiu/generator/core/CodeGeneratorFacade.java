...
public class CodeGeneratorFacade {
  ...
  public CodeGeneratorFacade(Vertx vertx, GeneratorContext context) {
    ...
    this.serviceBuilder =
        new ServiceBuilder(context.getFeatureConfig(), context.getPackageConfig());
    this.jServiceBuilder =
        new JServiceBuilder(context.getFeatureConfig(), context.getPackageConfig());
    this.controllerBuilder =
        new ControllerBuilder(context.getFeatureConfig(), context.getPackageConfig());
  ...
  public Future<List<String>> generateAll(List<String> tableNames) {
    return getMetadataReader().compose(reader -> generateAll(reader, tableNames));
  }
        ...
        promise -> {
          ...
          for (String tableName : tableNames) {
            Future<String> future =
                generateTable(reader, tableName)
                    .map(
                        result -> {
  ...
  public Future<List<String>> generateTable(String tableName) {
    return getMetadataReader().compose(reader -> generateTable(reader, tableName));
  }
        ...
        .compose(
            tableInfo -> {
              ...
              if (context.getFeatureConfig().isGenerateController()) {
                Future<String> controllerFuture =
              ...
              return Future.all(futures).map(v -> generatedFiles);
            });
  }
  ...
  private Future<String> generateJService(TableInfo tableInfo, EntityInfo entityInfo) {
    ...
    Future<String> interfaceFuture =
        generateCode(
            jServiceBuilder.getJServiceInterfaceTemplateName(),
            serviceInfo,
            getServiceOutputPath(serviceInfo));

    // 生成 JService 实现类
    EntityInfo serviceImplInfo = jServiceBuilder.buildJServiceImplementation(tableInfo, entityInfo);
    Future<String> implFuture =
        generateCode(
            jServiceBuilder.getJServiceImplementationTemplateName(),
            serviceImplInfo,
            getServiceOutputPath(serviceImplInfo));

    return Future.all(interfaceFuture, implFuture).map(v -> interfaceFuture.result());
  }
  ...
  private Future<String> generateTraditionalService(TableInfo tableInfo, EntityInfo entityInfo) {
    ...
    Future<String> interfaceFuture =
        generateCode("service", serviceInfo, getServiceOutputPath(serviceInfo));

    // 生成 Service 实现类
    EntityInfo serviceImplInfo = serviceBuilder.buildServiceImplementation(tableInfo, entityInfo);
    Future<String> implFuture =
        generateCode("service-impl", serviceImplInfo, getServiceOutputPath(serviceImplInfo));
  ...
  public Future<String> generateDto(TableInfo tableInfo) {
    ...
    Future<String> createFuture =
        generateCode("dto-create", createDtoInfo, getDtoOutputPath(createDtoInfo));

    // 生成更新请求 DTO
    EntityInfo updateDtoInfo = dtoBuilder.buildUpdateRequestDto(tableInfo, entityInfo);
    Future<String> updateFuture =
        generateCode("dto-update", updateDtoInfo, getDtoOutputPath(updateDtoInfo));

    // 生成响应 DTO
    EntityInfo responseDtoInfo = dtoBuilder.buildResponseDto(tableInfo, entityInfo);
    Future<String> responseFuture =
        generateCode("dto-response", responseDtoInfo, getDtoOutputPath(responseDtoInfo));

    // 生成 DTO 转换器
    EntityInfo converterInfo = dtoBuilder.buildDtoConverter(tableInfo, entityInfo);
    Future<String> converterFuture =
        generateCode("dto-converter", converterInfo, getDtoOutputPath(converterInfo));
  ...
  /** 生成代码 */
  private Future<String> generateCode(
      String templateName, EntityInfo entityInfo, String outputPath) {
        ...
        promise -> {
          try {
            ...
            if (context.getDatabaseConfig() != null) {
              ...
              if (configPath == null) {
                promise.fail(
                    new IllegalArgumentException("No database config or config file specified"));
                return;
              }
  ...
  private String getEntityOutputPath(EntityInfo entityInfo) {
    ...
    return Paths.get(
            context.getOutputConfig().getOutputPath(),
            packagePath,
            entityInfo.getClassName() + ".java")
        .toString();
  }
  ...
  private String getControllerOutputPath(EntityInfo controllerInfo) {
    ...
    return Paths.get(
            context.getOutputConfig().getOutputPath(),
            packagePath,
            controllerInfo.getClassName() + ".java")
        .toString();
  }
